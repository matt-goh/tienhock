...
export default function (pool) {
  ...
  async function checkDuplicateJobName(name, id = null) {
    const query = id
      ? "SELECT * FROM jellypolly.jobs WHERE name = $1 AND id != $2"
      : "SELECT * FROM jellypolly.jobs WHERE name = $1";
  ...
  async function checkDuplicateJobId(id) {
    const query = "SELECT * FROM jellypolly.jobs WHERE id = $1";
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      const query = "SELECT * FROM jellypolly.jobs";
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      const query = `
        INSERT INTO jellypolly.jobs (id, name, section)
        VALUES ($1, $2, $3)
        RETURNING *
      `;
    ...
    try {
      ...
      const payCodesResult = await pool.query(
        `SELECT jpc.id, jpc.pay_code_id, pc.description
         ...
         LEFT JOIN jellypolly.pay_codes pc ON pc.id = jpc.pay_code_id
         WHERE jpc.job_id = $1`,
        [id]
      );
      ...
      const staffsResult = await pool.query(
        `SELECT id, name FROM jellypolly.staffs WHERE job ? $1`,
        [id]
      );
      ...
      const dailyWorkLogsResult = await pool.query(
        `SELECT COUNT(*) as count FROM jellypolly.daily_work_log_entries WHERE job_id = $1`,
        [id]
      );

      // Check monthly_work_log_entries
      const monthlyWorkLogsResult = await pool.query(
        `SELECT COUNT(*) as count FROM jellypolly.monthly_work_log_entries WHERE job_id = $1`,
        [id]
      );
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      ...
      // Delete the job
      const deleteJobQuery = "DELETE FROM jellypolly.jobs WHERE id = $1";
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      const existingJobQuery = "SELECT * FROM jellypolly.jobs WHERE id = $1";
