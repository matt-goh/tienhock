// src/routes/jellypolly/locations.js
// Jelly Polly clone of src/routes/catalogue/entities/locations.js on the
// jellypolly schema. JP has no journal vouchers, so all location_account_mappings
// logic is dropped (dependencies return accounts: [] to keep the response shape).
...
export default function (pool) {
  ...
  // JP-scoped cache key — must not collide with the TH /api/locations route
  const JP_LOCATIONS_CACHE_KEY = `jp:${CACHE_KEYS.LOCATIONS}`;
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      const query = `
        INSERT INTO jellypolly.locations (id, name)
        VALUES ($1, $2)
        RETURNING *
      `;
  ...
  router.get("/", async (req, res) => {
    try {
      const cacheKey = JP_LOCATIONS_CACHE_KEY;
      ...
      const query = `SELECT * FROM jellypolly.locations ORDER BY id`;
    ...
    try {
      ...
      const jobMappingsResult = await pool.query(
        `SELECT jlm.id, jlm.job_id, j.name as job_name
         FROM jellypolly.job_location_mappings jlm
         LEFT JOIN jellypolly.jobs j ON j.id = jlm.job_id
      ...
      // JP has no journal vouchers / account mappings
      const accounts = [];
      const staffs = staffsResult.rows;

      const hasDependencies = jobs.length > 0 || staffs.length > 0;
      ...
      try {
        ...
        if (newId && newId !== id) {
          ...
          const existingCheck = await client.query(
            "SELECT id FROM jellypolly.locations WHERE id = $1",
            [newId]
          );
          ...
          // Insert new record
          await client.query(
            "INSERT INTO jellypolly.locations (id, name) VALUES ($1, $2)",
            [newId, name]
          );

          // Update job_location_mappings
          await client.query(
            "UPDATE jellypolly.job_location_mappings SET location_code = $1 WHERE location_code = $2",
            [newId, id]
          );

          // Update employee_job_location_exclusions (FK requires this before delete)
          await client.query(
            "UPDATE jellypolly.employee_job_location_exclusions SET location_code = $1 WHERE location_code = $2",
            [newId, id]
          );

          // Update staffs (location is stored as text or JSONB)
          await client.query(
            "UPDATE jellypolly.staffs SET location = $1 WHERE location::text = $2 OR location::text = '\"' || $2 || '\"'",
            [newId, id]
          );

          // Delete old record
          await client.query("DELETE FROM jellypolly.locations WHERE id = $1", [
            id,
          ]);
          ...
          const result = await client.query(
            "UPDATE jellypolly.locations SET name = $1 WHERE id = $2 RETURNING *",
            [name, id]
          );
  ...
  router.delete("/", async (req, res) => {
    ...
    try {
      ...
      for (const id of locationIds) {
        ...
        const staffsResult = await pool.query(
          `SELECT id, name FROM jellypolly.staffs WHERE location::text = $1 OR location::text = '"' || $1 || '"'`,
          [id]
        );
      ...
      const deleteQuery = `DELETE FROM jellypolly.locations WHERE id = ANY($1::text[]) RETURNING id`;
    ...
    try {
      const query = `
        SELECT
          ...
          jlm.is_active
        FROM jellypolly.jobs j
        LEFT JOIN jellypolly.job_location_mappings jlm ON j.id = jlm.job_id AND jlm.is_active = true
        LEFT JOIN jellypolly.locations l ON jlm.location_code = l.id
    ...
    try {
      const query = `
        SELECT
          ...
          ex.created_at,
          ex.created_by
        FROM jellypolly.employee_job_location_exclusions ex
        JOIN jellypolly.staffs s ON ex.employee_id = s.id
        JOIN jellypolly.jobs j ON ex.job_id = j.id
  ...
  router.get("/:id/exclusion-candidates", async (req, res) => {
    ...
    try {
      ...
      const query = `
        ...
        JOIN jellypolly.jobs j ON j.id = emp_job.value
        JOIN jellypolly.job_location_mappings jlm ON jlm.job_id = j.id AND jlm.is_active = true
        LEFT JOIN jellypolly.employee_job_location_exclusions ex
  ...
  router.post("/:id/exclusions", async (req, res) => {
    ...
    try {
      const query = `
        INSERT INTO jellypolly.employee_job_location_exclusions
  ...
  router.delete("/:id/exclusions/:exclusionId", async (req, res) => {
    ...
    try {
      const query = `
        DELETE FROM jellypolly.employee_job_location_exclusions
