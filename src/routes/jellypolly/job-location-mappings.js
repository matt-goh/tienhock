// src/routes/jellypolly/job-location-mappings.js
// Jelly Polly clone of src/routes/catalogue/job-location-mappings.js on the
// jellypolly schema.
...
async function getLocationMap(pool) {
  const result = await pool.query(
    "SELECT id, name FROM jellypolly.locations ORDER BY id"
  );
  const locationMap = {};
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      const query = `
        ...
        LEFT JOIN jellypolly.jobs j ON jlm.job_id = j.id
  ...
  router.get("/unmapped", async (req, res) => {
    try {
      const query = `
        ...
        LEFT JOIN jellypolly.job_location_mappings jlm ON j.id = jlm.job_id
    ...
    try {
      ...
      try {
        ...
        const locationCheck = await (transactionClient || pool).query(
          "SELECT id, name FROM jellypolly.locations WHERE id = $1",
          [location_code]
        );
        ...
        // Check if job exists
        const jobCheck = await (transactionClient || pool).query(
          "SELECT 1 FROM jellypolly.jobs WHERE id = $1",
          [job_id]
        );
        ...
        const existingCheck = await (transactionClient || pool).query(
          "SELECT 1 FROM jellypolly.job_location_mappings WHERE job_id = $1",
          [job_id]
        );
        ...
        const insertQuery = `
          INSERT INTO jellypolly.job_location_mappings (job_id, location_code, is_active)
          VALUES ($1, $2, $3)
          RETURNING *
        `;
  ...
  router.put("/:jobId", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const existingCheck = await (transactionClient || pool).query(
          "SELECT * FROM jellypolly.job_location_mappings WHERE job_id = $1",
          [jobId]
        );
        ...
        const updateQuery = `
          UPDATE jellypolly.job_location_mappings
          SET ${updates.join(", ")}
        ...
        if (!locationName && result.rows[0].location_code) {
          const locResult = await (transactionClient || pool).query(
            "SELECT name FROM jellypolly.locations WHERE id = $1",
  ...
  router.delete("/:jobId", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const deleteQuery = `
          DELETE FROM jellypolly.job_location_mappings
          WHERE job_id = $1
          RETURNING *
        `;
