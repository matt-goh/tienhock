  ...
  router.get("/all-mappings", async (req, res) => {
    try {
      ...
      const payCodeQuery = `
        ...
        FROM jellypolly.pay_codes ORDER BY updated_at DESC, created_at DESC
      `;
      ...
      const mappingQuery = `
        ...
        FROM jellypolly.employee_pay_codes epc
        JOIN jellypolly.pay_codes pc ON epc.pay_code_id = pc.id
      ...
      try {
        ...
        for (const entry of associations) {
          ...
          try {
            ...
            const checkQuery =
              "SELECT 1 FROM jellypolly.employee_pay_codes WHERE employee_id = $1 AND pay_code_id = $2";
            ...
            const insertQuery = `
              INSERT INTO jellypolly.employee_pay_codes (employee_id, pay_code_id, is_default, override_rate_biasa, override_rate_ahad, override_rate_umum)
        ...
        if (successCount > 0) {
          ...
          for (const employeeId of affectedEmployees) {
            const updateStaffQuery = `
              ...
              WHERE id = $1
            `;
            await (transactionClient || pool).query(updateStaffQuery, [employeeId]);
          }
          ...
          if (affectedPayCodes.size > 0) {
            const updatePayCodeQuery = `
              ...
              WHERE id = ANY($1::text[])
            `;
            await (transactionClient || pool).query(updatePayCodeQuery, [Array.from(affectedPayCodes)]);
          }
    ...
    try {
      ...
      try {
        ...
        for (const item of items) {
          ...
          try {
            const query = `
              DELETE FROM jellypolly.employee_pay_codes
              WHERE employee_id = $1 AND pay_code_id = $2
              RETURNING employee_id, pay_code_id
            `;
            const result = await (transactionClient || pool).query(query, [employee_id, pay_code_id]);
    ...
    try {
      ...
      try {
        ...
        const updateStaffQuery = `
          ...
          WHERE id = $1
        `;
        await (transactionClient || pool).query(updateStaffQuery, [employee_id]);
        ...
        const updatePayCodeQuery = `
          ...
          WHERE id = $1
        `;
        await (transactionClient || pool).query(updatePayCodeQuery, [pay_code_id]);
  ...
  router.put("/:employeeId/:payCodeId", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const query = `
        UPDATE jellypolly.employee_pay_codes
    ...
    `;
        ...
        const updatePayCodeQuery = `
          ...
          WHERE id = $1
        `;
        await (transactionClient || pool).query(updatePayCodeQuery, [payCodeId]);
    ...
    try {
      ...
      try {
        ...
        if (isByEmployee) {
          ...
          `;
          result = await (transactionClient || pool).query(updateQuery, [is_default, employee_id, pay_code_ids]);
          ...
          `;
          result = await (transactionClient || pool).query(updateQuery, [is_default, pay_code_id, employee_ids]);
        ...
        if (affectedEmployeeIds.length > 0) {
          const updateStaffQuery = `
            ...
            WHERE id = ANY($1::text[])
          `;
          await (transactionClient || pool).query(updateStaffQuery, [affectedEmployeeIds]);
        }
        ...
        if (affectedPayCodeIds.length > 0) {
          const updatePayCodesQuery = `
            ...
            WHERE id = ANY($1::text[])
          `;
          await (transactionClient || pool).query(updatePayCodesQuery, [affectedPayCodeIds]);
        }
  ...
  router.delete("/:employeeId/:payCodeId", async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        const result = await (transactionClient || pool).query(query, [employeeId, payCodeId]);
