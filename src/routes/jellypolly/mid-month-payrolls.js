// src/routes/jellypolly/mid-month-payrolls.js
// Jelly Polly mid-month advances. Mirrors src/routes/payroll/mid-month-payrolls.js
// but reads/writes jellypolly.mid_month_payrolls so JP records stay out of
// Tien Hock lists and reports. Changes auto-reprocess the employee's JP payroll
// (mid-month is deducted before rounding).
...
export default function (pool) {
  ...
  router.get("/", async (req, res) => {
    ...
    `;
      ...
      res.status(500).json({
        message: "Error fetching JP mid-month payrolls",
        error: error.message,
      });
    }
  });
  ...
  // (registered before /:id so "employee" isn't captured as an id)
  router.get("/employee/:employee_id/:year/:month", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching JP mid-month payroll by employee:", error);
      res.status(500).json({
        message: "Error fetching JP mid-month payroll",
        error: error.message,
      });
    }
  });
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching JP mid-month payroll:", error);
      res.status(500).json({
        message: "Error fetching JP mid-month payroll",
        error: error.message,
      });
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      const existingResult = await pool.query(
        `SELECT id FROM jellypolly.mid_month_payrolls
      ...
      // Create new mid-month payroll
      const insertQuery = `
        INSERT INTO jellypolly.mid_month_payrolls (
      ...
      console.error("Error creating JP mid-month payroll:", error);
      res.status(500).json({
        message: "Error creating JP mid-month payroll",
        error: error.message,
      });
    }
  });
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      const checkResult = await pool.query(
        `SELECT * FROM jellypolly.mid_month_payrolls WHERE id = $1`,
        [id]
      );
      ...
      values.push(id);
      const updateQuery = `
        UPDATE jellypolly.mid_month_payrolls
      ...
      await reprocessJPEmployeesSafe(pool, {
        ...
        employeeIds: [updateResult.rows[0].employee_id],
      });
      ...
      console.error("Error updating JP mid-month payroll:", error);
      res.status(500).json({
        message: "Error updating JP mid-month payroll",
        error: error.message,
      });
    }
  });
  ...
  router.put("/:id/status", async (req, res) => {
    ...
    try {
      ...
      // Cancelled advances stop counting toward the payroll deduction
      await reprocessJPEmployeesSafe(pool, {
        year: result.rows[0].year,
        month: result.rows[0].month,
        employeeIds: [result.rows[0].employee_id],
      });
      ...
      console.error("Error updating JP payment status:", error);
      res.status(500).json({
        message: "Error updating JP payment status",
        error: error.message,
      });
    }
  });
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `DELETE FROM jellypolly.mid_month_payrolls WHERE id = $1 RETURNING *`,
        [id]
      );
      ...
      res.status(500).json({
        message: "Error deleting JP mid-month payroll",
        error: error.message,
      });
    }
  });
