...
export default function (pool) {
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      const checkIdQuery = "SELECT 1 FROM jellypolly.pay_codes WHERE id = $1";
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      // 1. Check existence
      const checkExistQuery = "SELECT 1 FROM jellypolly.pay_codes WHERE id = $1";
      ...
      // const checkDescQuery = "SELECT id FROM jellypolly.pay_codes WHERE description = $1 AND id != $2";
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      ...
      const checkQuery =
        "SELECT 1 FROM jellypolly.job_pay_codes WHERE pay_code_id = $1 LIMIT 1";
      ...
      const query = "DELETE FROM jellypolly.pay_codes WHERE id = $1 RETURNING id"; // Only need ID back
      const result = await pool.query(query, [id]);
      if (result.rows.length === 0) {
        return res
          .status(404)
