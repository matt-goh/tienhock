    ...
    try {
      ...
      if (name) {
        const existingHeadResult = await pool.query(
          `SELECT head_staff_id FROM jellypolly.staffs
    ...
    try {
      ...
      try {
        ...
        const verifyResult = await client.query(
          "SELECT id, name FROM jellypolly.staffs WHERE id = $1 AND UPPER(TRIM(name)) = UPPER(TRIM($2))",
    ...
    try {
      ...
      try {
        ...
        if (newId && newId !== id) {
          ...
          const checkIdQuery = "SELECT id FROM jellypolly.staffs WHERE btrim(id) = btrim($1)";
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        await client.query(
          "DELETE FROM jellypolly.employee_leave_balances WHERE employee_id = $1",
          [id]
        );
        await client.query(
          "DELETE FROM jellypolly.employee_pay_codes WHERE employee_id = $1",
          [id]
        );

        // Now delete the staff member
        const query = "DELETE FROM jellypolly.staffs WHERE id = $1 RETURNING *";
