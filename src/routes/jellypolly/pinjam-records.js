// src/routes/jellypolly/pinjam-records.js
// Jelly Polly pinjam records. Mirrors src/routes/payroll/pinjam-records.js but
// reads/writes jellypolly.pinjam_records so JP records stay out of Tien Hock
// lists and reports.
...
export default function (pool) {
  ...
  router.get("/", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching JP pinjam records:", error);
      res.status(500).json({
        message: "Error fetching JP pinjam records",
        error: error.message,
      });
    }
  });

  // Get consolidated pinjam dashboard data (all data needed for the JP pinjam page)
  router.get("/dashboard", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching JP pinjam dashboard data:", error);
      res.status(500).json({
        message: "Error fetching JP pinjam dashboard data",
        error: error.message,
      });
    }
  });
  ...
  router.get("/summary", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        message: "Error fetching JP pinjam summary",
        error: error.message,
      });
    }
  });
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching JP pinjam record:", error);
      res.status(500).json({
        message: "Error fetching JP pinjam record",
        error: error.message,
      });
    }
  });
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      // Create new pinjam record
      const insertQuery = `
        INSERT INTO jellypolly.pinjam_records (
      ...
      console.error("Error creating JP pinjam record:", error);
      ...
      res.status(500).json({
        message: "Error creating JP pinjam record",
        error: error.message,
      });
    }
  });
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          const checkQuery = `
            SELECT id, amount FROM jellypolly.pinjam_records
          ...
          if (existingResult.rows.length > 0) {
            ...
            const updateQuery = `
              UPDATE jellypolly.pinjam_records
      ...
      await client.query('ROLLBACK');
      console.error("Error creating batch JP pinjam records:", error);
      res.status(500).json({
        message: "Error creating batch JP pinjam records",
        error: error.message,
      });
    } finally {
      client.release();
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      const checkResult = await pool.query(
        `SELECT * FROM jellypolly.pinjam_records WHERE id = $1`,
        [id]
      );
      ...
      console.error("Error updating JP pinjam record:", error);
      ...
      res.status(500).json({
        message: "Error updating JP pinjam record",
        error: error.message,
      });
    }
  });
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `DELETE FROM jellypolly.pinjam_records WHERE id = $1 RETURNING *`,
        [id]
      );
      ...
      console.error("Error deleting JP pinjam record:", error);
      res.status(500).json({
        message: "Error deleting JP pinjam record",
        error: error.message,
      });
    }
  });
