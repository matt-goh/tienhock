...
export default function (pool) {
  ...
  // Recalculate contributions for an employee payroll
  router.put("/:id/recalculate-contributions", async (req, res) => {
    const { id } = req.params;

    try {
      await pool.query("BEGIN");

      // Get employee payroll with items
      const payrollQuery = `
    ...
    `;
      ...
      // Get staff data for contribution calculation
      const staffQuery = `SELECT * FROM staffs WHERE id = $1`;
      const staffResult = await pool.query(staffQuery, [payroll.employee_id]);
      ...
      const [epfRates, socsoRates, sipRates] = await Promise.all([
        pool.query(
          "SELECT * FROM epf_rates WHERE is_active = true ORDER BY employee_type, wage_threshold NULLS LAST"
        ),
        pool.query(
      ...
      // Import the calculation service (you'll need to adjust import path)
      const {
        PayrollCalculationService,
      } = require("../../utils/payroll/payrollCalculationService");
      ...
      const grossPay = itemsResult.rows.reduce(
      ...
      // Update employee payroll totals
      const updatePayrollQuery = `
      UPDATE employee_payrolls
      SET gross_pay = $1, net_pay = $2, updated_at = CURRENT_TIMESTAMP
      WHERE id = $3
      RETURNING *
    `;
      const updatedPayroll = await pool.query(updatePayrollQuery, [
        grossPay,
        netPay,
        id,
      ]);
      ...
      await saveDeductions(pool, id, deductions);

      await pool.query("COMMIT");

      // Return updated payroll with deductions
      res.json({
        ...updatedPayroll.rows[0],
        gross_pay: parseFloat(updatedPayroll.rows[0].gross_pay),
        net_pay: parseFloat(updatedPayroll.rows[0].net_pay),
      ...
      res.status(500).json({
        message: "Error recalculating contributions",
        error: error.message,
      });
    }
  });
