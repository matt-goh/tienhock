// src/routes/excel/company-payment-export.js
// Parameterized clone of payment-export.js for the Jelly Polly and Green Target
// payroll schemas. Identical PBB text output; only the table schemas differ
// (JP keeps its own catalogue incl. staffs; GT payroll reads public.staffs).
import { Router } from "express";

export default function (pool, { payrollSchema, staffSchema }) {
  const router = Router();
    ...
    try {
      ...
      const query = `
        WITH employee_payroll_data AS (
          ...
          FROM ${payrollSchema}.employee_payrolls ep
          JOIN ${payrollSchema}.monthly_payrolls mp ON ep.monthly_payroll_id = mp.id
          JOIN ${staffSchema}.staffs s ON ep.employee_id = s.id
        ...
        mid_month_data AS (
          ...
          FROM ${payrollSchema}.mid_month_payrolls mmp
        ...
        pinjam_monthly_data AS (
          ...
          FROM ${payrollSchema}.pinjam_records pr
