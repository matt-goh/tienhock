// src/routes/greentarget/others-records.js
// Green Target "Others (Kerja Luar OT)" records on greentarget.others_records.
// Mirrors src/routes/payroll/others-records.js (same link_id / report_column
// multi-date logic), scoped to the greentarget schema.
...
export default function (pool) {
  ...
  /**
   * GET /greentarget/api/others-records
  ...
  router.get("/", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching GT others records:", error);
      res.status(500).json({
  ...
  /**
   * POST /greentarget/api/others-records
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      const insertQuery = `
        INSERT INTO greentarget.others_records (
      ...
      console.error("Error creating GT others record:", error);
      res.status(500).json({
  ...
  /**
   * PUT /greentarget/api/others-records/:id
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      const existing = await client.query(
        "SELECT * FROM greentarget.others_records WHERE id = $1",
        [id],
      );
      ...
      if (!row.link_id) {
        const result = await client.query(
          `UPDATE greentarget.others_records
             SET record_date = $1,
                 pay_code_id = $2,
                 description = $3,
                 rate = $4,
      ...
      if (newDates) {
        const siblings = await client.query(
          "SELECT id, record_date FROM greentarget.others_records WHERE link_id = $1",
          [row.link_id],
        );
        ...
        if (toDeleteIds.length > 0) {
          await client.query(
            "DELETE FROM greentarget.others_records WHERE id = ANY($1::int[])",
            [toDeleteIds],
          );
        }
      ...
      console.error("Error updating GT others record:", error);
      res.status(500).json({
  ...
  /**
   * DELETE /greentarget/api/others-records/:id
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      const existing = await client.query(
        "SELECT id, link_id FROM greentarget.others_records WHERE id = $1",
        [id],
      );
      if (existing.rows.length === 0) {
        return res.status(404).json({ message: "Others record not found." });
      ...
      if (row.link_id) {
        result = await client.query(
          "DELETE FROM greentarget.others_records WHERE link_id = $1 RETURNING id",
          [row.link_id],
        );
      } else {
        result = await client.query(
          "DELETE FROM greentarget.others_records WHERE id = $1 RETURNING id",
          [id],
        );
      }
      res.status(200).json({
      ...
      console.error("Error deleting GT others record:", error);
      res.status(500).json({
