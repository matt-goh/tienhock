// src/routes/greentarget/pinjam-records.js
// Green Target pinjam records. Mirrors src/routes/payroll/pinjam-records.js but
// reads/writes greentarget.pinjam_records so GT records stay out of Tien Hock
// lists and reports.
import { Router } from "express";
import { buildPinjamDashboardData } from "../payroll/pinjam-dashboard.js";
...
export default function (pool) {
  ...
  router.get("/", async (req, res) => {
    ...
    try {
      let query = `
        ...
        FROM greentarget.pinjam_records p
      ...
      console.error("Error fetching GT pinjam records:", error);
      res.status(500).json({
        message: "Error fetching GT pinjam records",
        error: error.message,
      });
    }
  });

  // Get consolidated pinjam dashboard data (all data needed for the GT pinjam page)
  router.get("/dashboard", async (req, res) => {
    ...
    try {
      ...
      const [
        ...
        pool.query(`
          SELECT p.*, s.name as employee_name
          FROM greentarget.mid_month_payrolls p
          LEFT JOIN public.staffs s ON p.employee_id = s.id
          WHERE p.year = $1 AND p.month = $2
        ...
        pool.query(`
          ...
          FROM greentarget.monthly_payrolls mp
          LEFT JOIN greentarget.employee_payrolls ep ON mp.id = ep.monthly_payroll_id
      ...
      console.error("Error fetching GT pinjam dashboard data:", error);
      res.status(500).json({
        message: "Error fetching GT pinjam dashboard data",
        error: error.message,
      });
    }
  });
  ...
  router.get("/summary", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        message: "Error fetching GT pinjam summary",
        error: error.message,
      });
    }
  });
    ...
    try {
      const query = `
        SELECT
          pr.*,
          s.name as employee_name
        FROM greentarget.pinjam_records pr
      ...
      console.error("Error fetching GT pinjam record:", error);
      res.status(500).json({
        message: "Error fetching GT pinjam record",
        error: error.message,
      });
    }
  });
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      // Create new pinjam record
      const insertQuery = `
        INSERT INTO greentarget.pinjam_records (
      ...
      console.error("Error creating GT pinjam record:", error);
      ...
      res.status(500).json({
        message: "Error creating GT pinjam record",
        error: error.message,
      });
    }
  });
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          const checkQuery = `
            SELECT id, amount FROM greentarget.pinjam_records
          ...
          if (existingResult.rows.length > 0) {
            ...
            const updateQuery = `
              UPDATE greentarget.pinjam_records
      ...
      await client.query('ROLLBACK');
      console.error("Error creating batch GT pinjam records:", error);
      res.status(500).json({
        message: "Error creating batch GT pinjam records",
        error: error.message,
      });
    } finally {
      client.release();
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      const checkResult = await pool.query(
        `SELECT * FROM greentarget.pinjam_records WHERE id = $1`,
        [id]
      );
      ...
      console.error("Error updating GT pinjam record:", error);
      ...
      res.status(500).json({
        message: "Error updating GT pinjam record",
        error: error.message,
      });
    }
  });
  ...
  router.delete("/:id", async (req, res) => {
    ...
    try {
      const result = await pool.query(
        `DELETE FROM greentarget.pinjam_records WHERE id = $1 RETURNING *`,
        [id]
      );
      ...
      res.status(500).json({
        message: "Error deleting GT pinjam record",
        error: error.message,
      });
    }
  });
