// src/routes/greentarget/e-caruman.js
// Green Target E-Caruman statutory exports (Phase 6): EPF (KWSP CSV),
// combined SOCSO + EIS/SIP + SKBBK (PERKESO fixed-width), PCB (LHDN CP39).
//
// Mirrors src/routes/payroll/e-caruman.js but on the greentarget schema. The
...
// stored in greentarget.payroll_settings (editable on the GT E-Caruman page).
...
export default function (pool) {
  ...
  // ---- Settings (GT employer registration codes, stored in payroll_settings) ----
  router.get("/settings", async (req, res) => {
    try {
      ...
      console.error("Error fetching GT e-caruman settings:", error);
  ...
  router.put("/settings", async (req, res) => {
    ...
    try {
      ...
      const upsert = `
        INSERT INTO greentarget.payroll_settings (setting_key, setting_value, description)
        VALUES ($1, $2, $3)
      ...
      console.error("Error saving GT e-caruman settings:", error);
  ...
  router.get("/preview", async (req, res) => {
    ...
    try {
      const epfQuery = `
        ...
        JOIN greentarget.payroll_deductions pd ON pd.employee_payroll_id = ep.id AND pd.deduction_type = 'epf'
      ...
      const socsoQuery = `
        ...
        JOIN greentarget.monthly_payrolls mp ON ep.monthly_payroll_id = mp.id
        JOIN public.staffs s ON ep.employee_id = s.id
        LEFT JOIN greentarget.payroll_deductions socso ON socso.employee_payroll_id = ep.id AND socso.deduction_type = 'socso'
        LEFT JOIN greentarget.payroll_deductions sip ON sip.employee_payroll_id = ep.id AND sip.deduction_type = 'sip'
        WHERE mp.month = $1 AND mp.year = $2
      ...
      const sipQuery = `
        ...
        JOIN greentarget.monthly_payrolls mp ON ep.monthly_payroll_id = mp.id
        JOIN public.staffs s ON ep.employee_id = s.id
        JOIN greentarget.payroll_deductions sip ON sip.employee_payroll_id = ep.id AND sip.deduction_type = 'sip'
        WHERE mp.month = $1 AND mp.year = $2
      ...
      const incomeTaxQuery = `
        ...
        JOIN greentarget.monthly_payrolls mp ON ep.monthly_payroll_id = mp.id
        JOIN public.staffs s ON ep.employee_id = s.id
        JOIN greentarget.payroll_deductions pcb ON pcb.employee_payroll_id = ep.id AND pcb.deduction_type = 'income_tax'
        WHERE mp.month = $1 AND mp.year = $2
      ...
      console.error("Error fetching GT e-caruman preview:", error);
  ...
  router.get("/epf/export", async (req, res) => {
    ...
    try {
      ...
      console.error("Error generating GT EPF export:", error);
  ...
  router.get("/socso-sip/export", async (req, res) => {
    ...
    try {
      ...
      console.error("Error generating GT SOCSO-SIP export:", error);
  ...
  router.get("/income-tax/export", async (req, res) => {
    ...
    try {
      ...
      console.error("Error generating GT Income Tax export:", error);
