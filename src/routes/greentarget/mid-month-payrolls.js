// src/routes/greentarget/mid-month-payrolls.js
// Green Target mid-month advances. Mirrors src/routes/payroll/mid-month-payrolls.js
// but reads/writes greentarget.mid_month_payrolls so GT records stay out of
...
export default function (pool) {
  ...
  router.get("/", async (req, res) => {
    ...
    `;
      ...
      res.status(500).json({
        message: "Error fetching GT mid-month payrolls",
        error: error.message,
      });
    }
  });
  ...
  router.get("/employee/:employee_id/:year/:month", async (req, res) => {
    ...
    try {
      const query = `
        ...
        FROM greentarget.mid_month_payrolls mmp
      ...
      console.error("Error fetching GT mid-month payroll by employee:", error);
      res.status(500).json({
        message: "Error fetching GT mid-month payroll",
        error: error.message,
      });
    }
  });
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      console.error("Error fetching GT mid-month payroll:", error);
      res.status(500).json({
        message: "Error fetching GT mid-month payroll",
        error: error.message,
      });
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      // Create new mid-month payroll
      const insertQuery = `
        INSERT INTO greentarget.mid_month_payrolls (
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      values.push(id);
      const updateQuery = `
        UPDATE greentarget.mid_month_payrolls
