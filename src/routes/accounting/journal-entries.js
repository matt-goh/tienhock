...
export default function (pool) {
  ...
  router.put("/:id", async (req, res) => {
    ...
    try {
      ...
      // Manually-keyed entries stay editable while active. Most system-generated
      // journals (receipts, purchases, payroll vouchers/payments…) are owned by
      // their source record — correct those by cancelling/regenerating from the
      // source screen. Sales ('S') journals are the exception: staff may take one
      // over and edit it directly. Doing so DETACHES it (manual_override below),
      // after which the invoice sync stops rebuilding it — see syncSalesJournalEntry.
      ...
      // An invoice-owned sales journal being edited becomes human-owned: force it
      // to stay typed 'S' (so the invoice-cancellation cascade in
      // cancelSalesJournalEntry still finds it) and flag it so the sync backs off.
      const isInvoiceOwned = existing.source_type === "invoice";
      const effectiveEntryType = isInvoiceOwned ? "S" : entry_type;
