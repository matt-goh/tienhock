...
export default function (pool) {
  ...
  // GET /api/production-entries/page-context - Bundled page data in one call:
  // entries (date or range, optional product filter) + per-scope worker orders
  // + per-product machine status. Used by the production records and entry pages.
    ...
    try {
      ...
      const [entriesResult, workerOrderResult, machineStatusResult] =
        await Promise.all([
          ...
          // Machine status is keyed per date+product and can exist before any
          // entry row does, so date mode queries it directly for the
          // requested products instead of relying on the entries join alone.
