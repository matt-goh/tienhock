...
export default function(pool) {
  ...
  router.post('/check-bulk-duplicates', async (req, res) => {
    ...
    try {
      ...
      const dbQuery = 'SELECT invoiceno FROM invoices WHERE invoiceno = ANY($1)';
      const dbResult = await pool.query(dbQuery, [invoiceNumbers]);

      // Check for duplicates in the provided list itself
      const duplicatesInList = invoiceNumbers.filter((item, index) => 
        invoiceNumbers.indexOf(item) !== index
      );
