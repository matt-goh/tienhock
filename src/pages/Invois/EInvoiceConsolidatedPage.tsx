...
const EInvoiceConsolidatedPage: React.FC = () => {
  ...
  const [selectedMonth, setSelectedMonth] = useState<MonthOption>(monthOptions[currentMonth]);
  ...
  return (
    <div className="flex flex-col mt-4">
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-default-200">
        <h2 className="text-xl font-medium mb-4">Select Month to Consolidate</h2>
            ...
            <Listbox value={selectedMonth} onChange={setSelectedMonth}>
              <div className="relative">
                <ListboxButton className="w-full rounded-lg border border-default-300 bg-white py-2 pl-3 pr-10 text-left focus:outline-none focus:border-default-500">
                  <span className="block truncate pl-2">{selectedMonth.name}</span>
        ...
        <p className="mt-4 text-default-500">
          This will consolidate all e-invoices from {selectedMonth.name} {currentYear} into a single submission batch.
        </p>
      </div>
    </div>
  );
