// src/pages/JellyPolly/Payroll/JPPackingCutiEntryPage.tsx
// Jelly Polly packing cuti entry, mirroring the Tien Hock packing cuti page.
// JP has a single packing job (JP_PACKING = Ice Polly & Jelly Cup packing), so
// there is no job-type prop. Records go into jellypolly.leave_records with a
...
const JOB_TYPE: string = "JP_PACKING";
...
const JPPackingCutiEntryPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await api.get(
        `/jellypolly/api/leave-management/packing-cuti?${query.toString()}`
      );
      ...
      if (fetchedWorkers.length > 0) {
        ...
        const balanceResponse = await api.get(
          `/jellypolly/api/leave-management/balances/batch?employeeIds=${employeeIds}&year=${year}`
        );
  ...
  const handleSave = async (): Promise<void> => {
    ...
    setIsSaving(true);
    try {
      await api.post("/jellypolly/api/leave-management/packing-cuti/batch", {
        job_type: JOB_TYPE,
        date: selectedDate,
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        <FormListbox
                          ...
                          disabled={isSaving}
                          className="w-44"
                          // Portalled so the table's horizontal scroll container
                          // does not clip the open dropdown.
                          anchor="bottom start"
                        />
                      </td>
...
export default JPPackingCutiEntryPage;
