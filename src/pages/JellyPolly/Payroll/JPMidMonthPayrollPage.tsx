// src/pages/JellyPolly/Payroll/JPMidMonthPayrollPage.tsx
// Jelly Polly mid-month advance entry page. Same flow as the Tien Hock
// Mid-month Payrolls page but scoped to JP payroll employees and the
...
interface JPMidMonthPayrollModalProps {
  ...
  currentYear: number;
  currentMonth: number;
  employees: JPPayrollEmployee[];
  editingPayroll: JPMidMonthPayroll | null;
}

const JPMidMonthPayrollModal: React.FC<JPMidMonthPayrollModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  currentYear,
...
const JPMidMonthPayrollPage: React.FC = () => {
  ...
  const [editingPayroll, setEditingPayroll] =
    useState<JPMidMonthPayroll | null>(null);
  ...
  const handleEdit = (payroll: JPMidMonthPayroll) => {
  ...
  const pinjamReportData: PinjamReportData[] = useMemo<PinjamReportData[]>(
    () =>
      payrolls.map(
        (payroll: JPMidMonthPayroll, index: number): PinjamReportData => {
                ...
                {payrolls.map((payroll) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-default-600 dark:text-gray-300">
                      {pinjamByEmp[payroll.employee_id]?.totalAmount
                        ? formatCurrency(
                            pinjamByEmp[payroll.employee_id]?.totalAmount ?? 0
                          )
                        : "-"}
...
export default JPMidMonthPayrollPage;
