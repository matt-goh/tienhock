...
const JPMonthlyLogListPage: React.FC<JPMonthlyLogListPageProps> = ({ jobType }) => {
  ...
  const fetchWorkLogs = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/jellypolly/api/monthly-work-logs?${params.toString()}`
      );
  ...
  const handleAddEntry = () => {
    navigate(
      `/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-monthly-entry`
    );
  };

  const handleViewLog = (log: MonthlyWorkLog) => {
    navigate(
      `/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-monthly/${log.id}`
    );
  };

  const handleEditLog = (log: MonthlyWorkLog) => {
    navigate(
      `/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-monthly/${log.id}/edit`
    );
  };
...
export default JPMonthlyLogListPage;
