// src/pages/Payroll/DailyLog/JPDailyLogSalesmanEntryPage.tsx
...
interface JPDailyLogSalesmanEntryPageProps {
  mode?: "create" | "edit";
  existingWorkLog?: any;
  onCancel?: () => void;
}
...
const JPDailyLogSalesmanEntryPage: React.FC<JPDailyLogSalesmanEntryPageProps> = ({
  mode = "create",
  existingWorkLog,
  onCancel,
}) => {
  ...
  }, [allStaffs, jpAssignedJobIdsByStaff, jobConfig]);
  ...
  const expandedEmployees = useMemo(() => {
    ...
    availableEmployees.forEach((employee) => {
      const configJobs = (
        jpAssignedJobIdsByStaff[employee.id] || []
      ).filter((jobId: string) => JOB_IDS.includes(jobId));
  ...
  }, [availableEmployees, jobs, JOB_IDS, jpAssignedJobIdsByStaff]);
  ...
  const salesmanEmployees = useMemo(() => {
    return expandedEmployees.filter(
      (emp: { jobType: string; id: string }) => emp.jobType === "JP_SALESMAN"
    );
  }, [expandedEmployees]);
  ...
  const salesmanIkutEmployees = useMemo(() => {
    return expandedEmployees.filter(
      (emp: { jobType: string }) => emp.jobType === "JP_SALESMAN_IKUT"
    );
  }, [expandedEmployees]);
  ...
  const salesmanProductStats = useMemo(() => {
    ...
    const selectedSalesmen = salesmanEmployees.filter((emp) =>
      employeeSelectionState.selectedJobs[emp.id]?.includes("JP_SALESMAN")
    );
  ...
  const handleBulkLeaveTypeChange = async (
    ...
    if (leaveType === "cuti_umum" && formData.dayType !== "Umum") {
      toast.error(t("Cuti Umum is only available on public holiday logs"));
      return;
    }
  ...
  const handleEmployeeSelection = (rowKey: string | undefined) => {
    ...
    if (isSelecting) {
      ...
      if (jobType === "JP_SALESMAN") {
        (followedBySalesman[employeeId] || []).forEach((ikutEmployeeId) => {
          rowKeysToCheck.push(`${ikutEmployeeId}-JP_SALESMAN_IKUT`);
        });
      }
      ...
      if (jobType === "JP_SALESMAN") {
        ...
        followedEmployees.forEach((ikutEmployeeId) => {
          ...
          if (wasSelected) {
            ...
            updatedSelectedJobs[ikutEmployeeId] =
              ikutCurrentSelectedJobs.filter((j) => j !== "JP_SALESMAN_IKUT");
          } else {
            // Selecting SALESMAN - also select following SALESMAN_IKUT employees (if not already selected)
            if (!ikutCurrentSelectedJobs.includes("JP_SALESMAN_IKUT")) {
              updatedSelectedJobs[ikutEmployeeId] = [
            ...
            if (!updatedJobHours[ikutEmployeeId]["JP_SALESMAN_IKUT"]) {
              updatedJobHours[ikutEmployeeId]["JP_SALESMAN_IKUT"] = getDefaultHours(formData.logDate);
            }
          }
        });
        ...
        if (!wasSelected && followedEmployees.length > 0) {
          setTimeout(() => {
            const salesmanRowKey = `${employeeId}-JP_SALESMAN`;
            ...
            followedEmployees.forEach((ikutEmployeeId) => {
              const ikutRowKey = `${ikutEmployeeId}-JP_SALESMAN_IKUT`;
  ...
  const fetchSalesmanProducts = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/jellypolly/api/invoices/salesman-products?salesmanIds=${salesmenIds.join(",")}&date=${dateString}`
      );
      ...
      Object.entries(responseData).forEach(([salesmanId, products]) => {
        const rowKey = `${salesmanId}-JP_SALESMAN`;
  ...
  useEffect(() => {
    ...
    const salesmanRowKeys = Object.entries(employeeSelectionState.selectedJobs)
      .flatMap(([employeeId, jobTypes]) =>
        jobTypes.filter((jt) => jt === "JP_SALESMAN").map((jt) => `${employeeId}-${jt}`)
      );
    ...
    if (salesmanId) {
      ...
      if (ikutEmployee) {
        setEmployeeSelectionState((prevState) => {
          ...
          if (!newSelectedJobs[ikutEmployee.id].includes("JP_SALESMAN_IKUT")) {
            newSelectedJobs[ikutEmployee.id].push("JP_SALESMAN_IKUT");
          }

          // Set default hours if not already set (5 for Saturday, 7 for other days)
          if (!newJobHours[ikutEmployee.id]["JP_SALESMAN_IKUT"]) {
            newJobHours[ikutEmployee.id]["JP_SALESMAN_IKUT"] = getDefaultHours(formData.logDate);
          }
        ...
        setEmployeeSelectionState((prevState) => {
          ...
          if (newSelectedJobs[ikutEmployee.id]) {
            newSelectedJobs[ikutEmployee.id] = newSelectedJobs[
              ikutEmployee.id
            ].filter((jobType) => jobType !== "JP_SALESMAN_IKUT");
          }
  ...
  useEffect(() => {
    ...
    Object.entries(salesmanIkutRelations).forEach(([ikutRowKey, salesmanId]) => {
      ...
      const salesmanRowKey = `${salesmanId}-JP_SALESMAN`;
  ...
  useEffect(() => {
    ...
    setEmployeeSelectionState((prev) => {
      ...
      excludedEmployees.forEach((employeeId) => {
        if (updatedSelectedJobs[employeeId]?.includes("JP_SALESMAN")) {
          updatedSelectedJobs[employeeId] = updatedSelectedJobs[
            employeeId
          ].filter((jobType) => jobType !== "JP_SALESMAN");
          needsUpdate = true;
        }
      });
  ...
  useEffect(() => {
    if (
      ...
      existingWorkLog.employeeEntries.forEach((entry: any) => {
        ...
        if (entry.job_id === "JP_SALESMAN_IKUT") {
  ...
  return (
    <div className="space-y-4">
            ...
            <>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg border border-default-200 dark:border-gray-700 shadow-sm">
                <div className="px-4 py-2 border-b border-default-200 dark:border-gray-700 flex items-center justify-between">
                  ...
                  >
                    {jobs.find((j) => j.id === "JP_SALESMAN")?.name || "Salesman"}
                  </SafeLink>
                  {salesmanProductStats.total > 0 && (
                    <div className="flex items-center gap-3 text-xs text-default-500 dark:text-gray-400 truncate min-w-0 flex-1 justify-end ml-4">
                      ...
                      <span className="flex-shrink-0">
                        {t("Mee: {{count}} · BH: {{bihun}}", {
                          count: salesmanProductStats.totalMee,
                      ...
                      <span className="font-medium flex-shrink-0">
                        {t("Total: {{count}}", {
                          count: salesmanProductStats.total,
                        })}
                      </span>
                    </div>
                  )}
      ...
      <ManageActivitiesModal
        ...
        logDate={formData.logDate}
        isDoubled={
          selectedEmployee?.jobType === "JP_SALESMAN_IKUT"
...
export default JPDailyLogSalesmanEntryPage;
