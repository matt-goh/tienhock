...
const JPPayrollDetailsPage: React.FC = () => {
  ...
  const fetchPayrollDetails = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await api.get(`/jellypolly/api/employee-payrolls/${id}`);
      setPayroll(response);
    } catch (error) {
  ...
  const handleDeleteItem = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/jellypolly/api/employee-payrolls/items/${itemToDelete.id}`);
  ...
  // Convert JP payroll format to EmployeePayroll format for the shared
  ...
  const buildPayslipData = (payroll: JPEmployeePayroll) => {
    const { pdfPayroll } = buildJPPayslipPayroll(payroll);
  ...
  if (!payroll) {
    return (
      <div className="p-6">
        <BackButton onClick={() => navigate("/jellypolly/payroll")} />
  ...
  return (
    <div className="pb-6 space-y-3">
      ...
      {/* Combined take-home across companies (renders only for dual-company staff) */}
