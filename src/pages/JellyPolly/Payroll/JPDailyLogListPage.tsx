...
interface JPDailyLogListPageProps {
  jobType: string;
}
...
const JPDailyLogListPage: React.FC<JPDailyLogListPageProps> = ({ jobType }) => {
  ...
  const fetchWorkLogs = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/jellypolly/api/daily-work-logs?${params.toString()}`
      );
  ...
  const handleAddEntry = () => {
    navigate(`/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-entry`);
  };

  const handleViewLog = (log: WorkLog) => {
    navigate(`/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-production/${log.id}`);
  };

  const handleEditLog = (log: WorkLog) => {
    navigate(`/jellypolly/payroll/${jobType.toLowerCase().replace("_", "-")}-production/${log.id}/edit`);
  };
...
export default JPDailyLogListPage;
