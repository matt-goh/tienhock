...
const JPStaffFormPage: React.FC = () => {
  ...
  const handleConfirmDelete = async () => {
    if (id) {
      try {
        await api.delete(`/jellypolly/api/staffs/${id}`);
  ...
  const fetchSameNameStaff = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/jellypolly/api/staffs/same-name/${id}`);
  ...
  const handleHeadStaffChange = async (newHeadStaffId: string) => {
    ...
    try {
      await api.put("/jellypolly/api/staffs/set-head", {
  ...
  const handleAddSameNameStaff = () => {
    ...
    navigate("/jellypolly/catalogue/staff/new", { state: { prefillData } });
  };
    ...
    try {
      if (isEditMode) {
        if (id !== formData.id) {
          ...
          await api.put(`/jellypolly/api/staffs/${id}`, dataToSend);
        } else {
        ...
        await api.post("/jellypolly/api/staffs", dataToSend);
      }
                            ...
                            {staff.id !== id && (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (!settingHeadStaff) {
                                    navigate(`/jellypolly/catalogue/staff/${staff.id}`);
                                  }
                                }}
...
export default JPStaffFormPage;
