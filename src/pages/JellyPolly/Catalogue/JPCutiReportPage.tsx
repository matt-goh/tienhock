// src/pages/Payroll/JPCutiReportPage.tsx
...
const JPCutiReportPage: React.FC = () => {
    ...
    const fetchLeaveData = async () => {
      ...
      try {
        const [balanceRes, recordsRes] = await Promise.all([
          api.get(
            `/jellypolly/api/leave-management/balances/${selectedStaffId}/${currentYear}`
          ),
          api.get(
            `/jellypolly/api/leave-management/records/${selectedStaffId}/${currentYear}`
          ),
        ]);
  ...
  const generateSinglePDF = async (action: "download" | "print") => {
    ...
    try {
      ...
      toast.success(
        t(
          action === "download"
            ? "Leave report downloaded successfully"
            : "Leave report generated for printing successfully"
        )
      );
    } catch (error) {
  ...
  const generateBatchPDF = async (action: "download" | "print") => {
    ...
    try {
      ...
      const batchResponse = await api.post('/jellypolly/api/leave-management/batch-reports', {
        employeeIds,
        year: currentYear
      });
      ...
      toast.success(
        t("Batch leave report generated successfully ({{total}} employees)", {
          total: validEmployeeData.length,
        })
      );
    } catch (error) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-4 border-t border-default-200 dark:border-gray-700 pt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-default-800 dark:text-gray-100">
                {t("{{total}} years", { total: yearsOfService })}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="bg-sky-50 dark:bg-sky-900/30 p-4 rounded-lg border border-sky-200 dark:border-sky-800">
            ...
            <div className="flex justify-between items-baseline mt-2">
              ...
              <span className="text-sm text-sky-500 dark:text-sky-400">
                {t("/ {{total}} days", {
                  total: balances.cuti_tahunan_total,
                })}
              </span>
            </div>
          </div>
          <div className="bg-amber-50 dark:bg-amber-900/30 p-4 rounded-lg border border-amber-200 dark:border-amber-800">
            ...
            <div className="flex justify-between items-baseline mt-2">
              ...
              <span className="text-sm text-amber-500 dark:text-amber-400">
                {t("/ {{total}} days", {
                  total: balances.cuti_sakit_total,
                })}
              </span>
            </div>
          </div>
          <div className="bg-emerald-50 dark:bg-emerald-900/30 p-4 rounded-lg border border-emerald-200 dark:border-emerald-800">
            ...
            <div className="flex justify-between items-baseline mt-2">
              ...
              <span className="text-sm text-emerald-500 dark:text-emerald-400">
                {t("/ {{total}} days", {
                  total: balances.cuti_umum_total,
                })}
              </span>
            </div>
          </div>
          <div className="bg-violet-50 dark:bg-violet-900/30 p-4 rounded-lg border border-violet-200 dark:border-violet-800">
            ...
            <div className="flex justify-between items-baseline mt-2">
              ...
              <span className="text-sm text-violet-500 dark:text-violet-400">
                {t("/ {{total}} days", {
                  total: balances.cuti_rawatan_total,
                })}
              </span>
            </div>
          </div>
    ...
    const calculateRunningBalances = () => {
      ...
      for (let month = 1; month <= 12; month++) {
        ...
        results[month] = {
          ...
          rawatanCumulative: cumulativeRawatan,
        };
      }
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-default-200 dark:border-gray-700">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-default-200 dark:border-gray-700 rounded-lg">
            <thead>
              {/* Main header row with leave type categories */}
              <tr className="bg-default-100 dark:bg-gray-800">
                <th
                  rowSpan={2}
                  className="py-4 px-4 text-left text-sm font-semibold text-default-700 dark:text-gray-200 uppercase align-middle border-r-2 border-default-300 dark:border-gray-600 bg-default-50 dark:bg-gray-900/50"
                >
                  {t("Month")}
                </th>
                <th
                ...
                <th
                  colSpan={3}
                  className="py-3 px-4 text-center text-sm font-semibold text-emerald-800 dark:text-emerald-300 uppercase border-b border-r-2 border-default-300 dark:border-gray-600 bg-emerald-100 dark:bg-emerald-900/40"
                >
                  <div className="flex items-center justify-center gap-2">
                ...
                <th
                  colSpan={3}
                  className="py-3 px-4 text-center text-sm font-semibold text-violet-800 dark:text-violet-300 uppercase border-b bg-violet-100 dark:bg-violet-900/40"
                >
                  <div className="flex items-center justify-center gap-2">
              ...
              <tr className="bg-default-50 dark:bg-gray-900/50">
                ...
                <th className="py-3 px-3 text-center text-xs font-medium text-emerald-700 dark:text-emerald-300 uppercase border-r-2 border-default-300 dark:border-gray-600 bg-emerald-50 dark:bg-emerald-900/30">
                  {t("Balance")}
                </th>
                {/* Cuti Rawatan sub-headers */}
                <th className="py-3 px-3 text-center text-xs font-medium text-violet-700 dark:text-violet-300 uppercase border-r border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-900/30">
                  {t("Days Used")}
                </th>
                <th className="py-3 px-3 text-center text-xs font-medium text-violet-700 dark:text-violet-300 uppercase border-r border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-900/30">
                  {t("Amount")}
                </th>
                <th className="py-3 px-3 text-center text-xs font-medium text-violet-700 dark:text-violet-300 uppercase bg-violet-50 dark:bg-violet-900/30">
                  {t("Balance")}
                </th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-800">
              {Object.entries(monthlySummary).map(([month, summary], index) => {
                ...
                return (
                  ...
                  >
                    {/* Month column */}
                    <td className="py-4 px-4 whitespace-nowrap text-sm font-semibold text-default-800 dark:text-gray-100 border-r-2 border-default-300 dark:border-gray-600 bg-default-50 dark:bg-gray-900/50">
                      {t(getMonthName(monthNum))}
                    </td>
                    ...
                    <td className="py-4 px-3 whitespace-nowrap text-sm text-center font-bold text-emerald-700 dark:text-emerald-300 border-r-2 border-default-300 dark:border-gray-600 bg-emerald-50 dark:bg-emerald-900/20">
                      <span
                        className={`px-3 py-1 rounded-full ${
                    ...
                    {/* Cuti Rawatan columns */}
                    <td className="py-4 px-3 whitespace-nowrap text-sm text-center text-default-700 dark:text-gray-200 border-r border-violet-100 dark:border-violet-800/50 bg-violet-25 dark:bg-violet-900/10">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    <td className="py-4 px-3 whitespace-nowrap text-sm text-center font-bold text-violet-700 dark:text-violet-300 bg-violet-50 dark:bg-violet-900/20">
                      <span
                        className={`px-3 py-1 rounded-full ${
                          balances.rawatanBalance < 0
                            ? "bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300"
            ...
            <tfoot className="bg-default-100 dark:bg-gray-800 border-t-2 border-default-300 dark:border-gray-600">
              <tr>
                <td className="py-4 px-4 whitespace-nowrap text-sm font-bold text-default-800 dark:text-gray-100 border-r-2 border-default-300 dark:border-gray-600 bg-default-100 dark:bg-gray-800">
                  {t("TOTAL")}
                </td>
                ...
                <td className="py-4 px-3 whitespace-nowrap text-sm text-center font-bold text-emerald-800 dark:text-emerald-300 border-r-2 border-default-300 dark:border-gray-600 bg-emerald-100 dark:bg-emerald-900/40">
                  <span
                    className={`px-3 py-1 rounded-full font-bold ${
                ...
                {/* Cuti Rawatan totals */}
                <td className="py-4 px-3 whitespace-nowrap text-sm text-center font-bold text-violet-800 dark:text-violet-300 border-r border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-900/30">
                  <span className="px-2 py-1 rounded-full bg-violet-200 dark:bg-violet-900/50 text-violet-900 dark:text-violet-300">
                    {Object.values(monthlySummary).reduce(
                ...
                <td className="py-4 px-3 whitespace-nowrap text-sm text-center font-bold text-violet-800 dark:text-violet-300 bg-violet-100 dark:bg-violet-900/40">
                  <span
                    className={`px-3 py-1 rounded-full font-bold ${
                      leaveBalances.cuti_rawatan_total -
                        (leaveTaken.cuti_rawatan || 0) <
                      0
                        ? "bg-rose-200 text-rose-900 dark:bg-rose-900/50 dark:text-rose-300"
                        : "bg-violet-200 text-violet-900 dark:bg-violet-900/50 dark:text-violet-300"
                    }`}
                  >
                    {leaveBalances.cuti_rawatan_total -
                      (leaveTaken.cuti_rawatan || 0)}
...
export default JPCutiReportPage;
