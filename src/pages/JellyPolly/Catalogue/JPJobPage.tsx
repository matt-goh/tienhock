// src/pages/JellyPolly/Catalogue/JPJobPage.tsx
...
// JP jobs have their own section set (no sections table)
const JP_SECTION_OPTIONS = [
  "OFFICE",
  "MAINTENANCE",
  "SALESMAN",
  "ICE_POLLY",
...
const JPJobPage: React.FC = () => {
  ...
  const { jobs, loading: loadingJobs, refreshJobs } = useJPJobsCache();
  const { staffs, loading: loadingStaffs, refreshStaffs } = useJPStaffsCache();
  ...
  const handleAddPayCodeToJob = async (payCodeId: string) => {
    ...
    try {
      await api.post(`${JP_API_BASE}/job-pay-codes`, {
  ...
  const handleJobSelection = useCallback(
    (selection: Job | null | undefined) => {
      if (selection === undefined) {
        ...
        if (selection) {
          navigate(`/jellypolly/catalogue/job?id=${selection.id}`, { replace: true });
        } else {
          navigate(`/jellypolly/catalogue/job`, { replace: true });
        }
      }
    },
  ...
  const handleJobCardClick = (job: Job) => {
    ...
    // Update URL with selected job ID
    navigate(`/jellypolly/catalogue/job?id=${job.id}`, { replace: true });
  };
  ...
  const handleJobAdded = useCallback(
    async (newJobData: Omit<Job, "newId">) => {
      try {
        ...
        const response = await api.post(`${JP_API_BASE}/jobs`, jobToSend);
  ...
  const handleDeleteSelectedJobClick = useCallback(async () => {
    ...
    try {
      const response = await api.get(`${JP_API_BASE}/jobs/${selectedJob.id}/dependencies`);
  ...
  return (
    <div className="space-y-4">
              ...
              {loadingStaffs ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {associatedStaff.map((staff) => (
                    <Link
                      key={staff.id}
                      to={`/jellypolly/catalogue/staff/${staff.id}`}
                      ...
                      paginatedPayCodes.map((detail) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <Link
                                to={`/jellypolly/catalogue/pay-codes?desc=${detail.pay_code_id}`}
      ...
      <NewJobModal
        ...
        sectionOptions={JP_SECTION_OPTIONS}
      />
      <NewPayCodeModal
