...
export const validateSupplierPaymentDraft = (
  ...
  if (!draft.payment_date) {
    return t ? t("Payment date is required") : "Payment date is required";
  }
  ...
  if (!(amount > 0)) {
    return t
      ? t("Payment amount must be greater than zero")
      : "Payment amount must be greater than zero";
  }
  if (amount - outstandingAmount > 0.005) {
    const formatted: string = formatSupplierPaymentAmount(outstandingAmount);
    return t
      ? t("Payment amount exceeds outstanding balance ({{amount}})", {
          amount: formatted,
        })
      : `Payment amount exceeds outstanding balance (${formatted})`;
  }
  if (draft.payment_method !== "cash" && !draft.bank_account) {
    return t
      ? t("Bank account is required for non-cash payments")
