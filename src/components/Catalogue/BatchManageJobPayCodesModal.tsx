// src/components/Catalogue/BatchManageJobPayCodesModal.tsx
...
interface BatchManageJobPayCodesModalProps {
  ...
  job: Job | null;
  allPayCodes: PayCode[];
  currentPayCodeDetails: JobPayCodeDetails[];
  onComplete: () => Promise<void>;
}

const BatchManageJobPayCodesModal: React.FC<BatchManageJobPayCodesModalProps> = ({
  isOpen,
  onClose,
  job,
  allPayCodes,
  ...
  }, [isOpen, job, currentPayCodeDetails]);

  // Assigned pay codes (sorted alphabetically)
  const assignedPayCodes = useMemo(() => {
    const assigned = allPayCodes.filter((pc) => selectedPayCodes.has(pc.id));
  ...
  }, [allPayCodes, selectedPayCodes, assignedSearch]);

  // Available pay codes (not assigned, sorted alphabetically)
  const availablePayCodes = useMemo(() => {
    const available = allPayCodes.filter((pc) => !selectedPayCodes.has(pc.id));
    ...
    return available
      ...
      .sort((a, b) => a.description.localeCompare(b.description));
  }, [allPayCodes, selectedPayCodes, availableSearch]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Handle additions
      if (paycodesToAdd.length > 0) {
        const associations = paycodesToAdd.map((payCodeId) => ({
          job_id: job.id,
      ...
      // Handle removals
      if (paycodesToRemove.length > 0) {
        const items = paycodesToRemove.map((payCodeId) => ({
          job_id: job.id,
          pay_code_id: payCodeId,
        }));
      ...
      if (defaultsToSet.length > 0) {
        const items = defaultsToSet.map((payCodeId) => ({
          job_id: job.id,
          pay_code_id: payCodeId,
        }));
      ...
      if (defaultsToClear.length > 0) {
        const items = defaultsToClear.map((payCodeId) => ({
          job_id: job.id,
          pay_code_id: payCodeId,
        }));
...
export default BatchManageJobPayCodesModal;
