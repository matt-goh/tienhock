...
interface PayrollUnifiedTableProps {
  // "groups": render job groups; "flat": render a single ordered employee list
  // split across two columns on large screens.
  variant: "groups" | "flat";
  jobGroups?: JobGroup[];
  flatEmployees?: EmployeePayroll[];
  ...
  onProcessEmployee: (employeePayroll: EmployeePayroll) => void;
  isProcessing: boolean;
  processingEmployeePayrollId: number | null;
...
const PayrollUnifiedTable: React.FC<PayrollUnifiedTableProps> = ({
  ...
  const renderEmployeeRow = (employeePayroll: EmployeePayroll) => (
    <EmployeePayrollTableRow
      ...
      onProcess={onProcessEmployee}
      isProcessingDisabled={isProcessing}
      isProcessingThis={
        employeePayroll.id != null &&
        employeePayroll.id === processingEmployeePayrollId
      }
  ...
  // Flat variant: a single ordered list of all employees.
  if (variant === "flat") {
    return (
            ...
            {jobGroups.map(({ jobType, employees }) => {
              ...
              return (
                <React.Fragment key={jobType}>
                  ...
                  {/* Employee Rows */}
                  {isExpanded && employees.map(renderEmployeeRow)}
                </React.Fragment>
              );
            })}
          </tbody>
