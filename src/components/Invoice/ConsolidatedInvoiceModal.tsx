...
const ConsolidatedInvoiceModal: React.FC<ConsolidatedInvoiceModalProps> = ({
  ...
  const handleSubmitConsolidated = async () => {
    ...
    try {
      ...
      setSubmissionResults({
        success: false,
        message: error.message || t("Failed to submit consolidated invoice"),
        rejectedDocuments: [
          {
  ...
  const handleUpdateConsolidatedStatus = async (id: string) => {
    ...
    const toastId = toast.loading(t("Checking status for {{id}}...", { id }));
    try {
      ...
      if (response.success) {
        toast.success(
          response.message ||
            t("Status check complete for {{id}}.", { id }),
          {
            id: toastId,
          }
        );
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 flex justify-center items-center p-4 backdrop-blur-sm -top-4">
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-7xl rounded-xl shadow-xl flex flex-col max-h-[calc(100vh-40px)] animate-fade-in-scale">
            ...
            <button
              className={`px-4 py-1.5 text-sm rounded-md transition-colors duration-150 ${
                activeTab === "history"
                  ...
                  : "text-default-600 dark:text-gray-400 hover:text-default-900 dark:hover:text-gray-200 hover:bg-default-50 dark:hover:bg-gray-800"
              }`}
              onClick={() => setActiveTab("history")}
            >
              {t("Consolidation History ({{total}})", {
                total: consolidationHistory.length,
              })}
            </button>
            <button
              className={`px-4 py-1.5 text-sm rounded-md transition-colors duration-150 ${
        ...
        <div className="px-5 py-4 border-b border-default-200 dark:border-gray-700 flex flex-col bg-default-50/60 dark:bg-gray-900/30 flex-shrink-0">
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              <div className="mr-3">
                <div className="text-sm font-medium text-default-800 dark:text-gray-200">
                  {t("Auto Consolidation (Monthly)")}
                </div>
                <p className="text-xs text-default-500 dark:text-gray-400 mt-0.5">
                  {t(
                    "Automatically consolidate eligible invoices during days 3-7 of each month for the previous month's invoices."
                  )}
                </p>
              </div>
            </div>
            <div className="relative">
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${
                  ...
                  isAutoConsolidationEnabled ? "bg-sky-600 dark:bg-sky-500" : "bg-default-200 dark:bg-gray-600"
                }`}
                  ...
                  <div className="mb-3">
                    ...
                    {windowInfo.inWindow ? (
                      windowInfo.existingConsolidation ? (
                        <div className="text-xs text-default-600 dark:text-gray-300 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                          <div className="flex items-center mb-2">
                            ...
                            <strong className="text-blue-800 dark:text-blue-300">
                              {t("Consolidation Already Completed")}
                            </strong>
                          </div>
                          <div className="mb-1">
                          ...
                          <div className="mb-1">
                            ...
                            {t("Valid consolidated e-invoice already submitted")}
                          </div>
                          <div>
                        ...
                        <div className="text-xs text-default-600 dark:text-gray-300 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
                          ...
                          <div className="mb-1">
                            <strong>{t("Day:")}</strong>{" "}
                            {t(
                              "{{day}} of 5 in consolidation window (days 3-7)",
                              { day: (windowInfo.dayInWindow || 0) - 2 }
                            )}
                          </div>
                          <div className="mb-2">
                          ...
                          <div className="mt-3 p-3 bg-white dark:bg-gray-800 rounded border border-green-300 dark:border-green-700">
                            <h5 className="font-medium text-green-800 dark:text-green-300 mb-2 text-xs">
                              {t("Auto-Consolidation Preview")}
                            </h5>
                            {isLoadingAutoPreview ? (
                              ...
                              <div className="text-xs space-y-1">
                                <div>
                                  <strong>{t("Eligible Invoices:")}</strong>{" "}
                                  {autoConsolidationPreview.invoiceCount}
                                </div>
                                <div>
                                  <strong>{t("Total Excluding Tax:")}</strong> RM{" "}
                                ...
                                <div>
                                  <strong>{t("Total Payable:")}</strong> RM{" "}
                              ...
                              <div className="text-xs text-gray-500">
                                {t("No eligible invoices for auto-consolidation")}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="text-xs text-default-600 dark:text-gray-300 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                        ...
                        <div>
                          {t(
                            "Auto-consolidation runs during days 3-7 of each month for the previous month's eligible invoices."
                          )}
                        </div>
                      </div>
                    )}
          ...
          {activeTab === "eligible" ? (
            ...
            <div className="space-y-3">
              ...
              {!isLoadingEligible && error && (
                <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-700 rounded-lg p-4 text-rose-700 dark:text-rose-300">
                  <div className="flex items-center">
                    <IconAlertTriangle
                ...
                <div className="border border-default-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
                  ...
                  <div className="bg-default-50/70 dark:bg-gray-900/50 p-3 border-b border-default-200 dark:border-gray-700 flex flex-wrap items-center gap-x-4 gap-y-2 group">
                    ...
                    >
                      ...
                      <span className="ml-2 text-sm font-medium text-default-700 dark:text-gray-300 hidden sm:inline">
                        {selectedInvoices.size === eligibleInvoices.length
                          ? t("Deselect All")
                          : t("Select All")}
                      </span>
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-right">
                                {item.consolidated_invoices?.length ? (
                                  <button
                                    ...
                                    title={t(
                                      "View and print the invoices in this consolidation"
                                    )}
                                  >
                                    {item.consolidated_invoices.length}
                                  </button>
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm font-mono max-w-[150px] truncate">
                                ...
                                item.long_id ? (
                                  <a
                                    ...
                                    title={t("Open on the MyInvois portal")}
                                  >
                                    {item.uuid}
                                  </a>
                                ) : (
