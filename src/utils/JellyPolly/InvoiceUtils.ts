...
export const getInvoices = async (
  ...
  try {
    ...
    const response = await api.get(`/jellypolly/api/invoices${queryString}`);
...
export const cancelInvoice = async (
  ...
  try {
    ...
    const response = await api.delete(`/jellypolly/api/invoices/${id}`);
...
export const getInvoiceById = async (
  ...
  try {
    const inv = await api.get(`/jellypolly/api/invoices/${id}`);
...
export const getInvoiceDetailsBundle = async (
  ...
  try {
    const inv = await api.get(
      `/jellypolly/api/invoices/${id}?include=payments,adjustments`
    );
...
export const checkDuplicateInvoiceNo = async (
  ...
  try {
    const response = await api.get(`/jellypolly/api/invoices/${invoiceNo}`);
...
export const syncCancellationStatus = async (invoiceId: string) => {
  try {
    const response = await api.post(
      `/jellypolly/api/einvoice/cancelled/${invoiceId}/sync`
    );
    return response;
  } catch (error) {
    console.error(
  ...
  try {
    ...
    toast.error(
      error.response?.data?.message ||
        (error instanceof Error
          ? error.message
          : i18n.t("Failed to confirm payment", { ns: "misc" }))
    );
    throw new Error(errorMessage);
  }
};
...
export const getPaymentsForInvoice = async (
  ...
  try {
    const response = await api.get(
      `/jellypolly/api/payments?invoice_id=${invoiceId}${
