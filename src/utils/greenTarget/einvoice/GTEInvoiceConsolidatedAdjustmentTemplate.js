// src/utils/greenTarget/einvoice/GTEInvoiceConsolidatedAdjustmentTemplate.js
// Consolidated UBL XML for Green Target Credit / Debit / Refund Notes that
// adjust a single consolidated invoice. Each child adjustment doc becomes one
// InvoiceLine. BillingReference points at the parent consolidated invoice's
// UUID. Phase 7 — mirrors EInvoiceConsolidatedAdjustmentTemplate.js with GT
...
function createLineXml(child, index) {
  const subtotal = Number(child.amount_before_tax || 0);
  ...
  <cac:InvoiceLine>
    ...
    <cac:Item>
      <cbc:Description>${escapeXml(
        `Adjustment ${child.id} for Invoice ${child.original_invoice_number}` +
...
/**
 * Build the consolidated GT adjustment-doc UBL XML.
 *
 * @param {Object} args
 * @param {string} args.consolidatedId  - e.g. CON-GT-CN-202605-1
 * @param {'credit_note'|'debit_note'|'refund_note'} args.type
 * @param {Array} args.childDocs  - rows from greentarget.adjustment_documents
 *   (all same type, all referencing the same parent consolidated invoice)
 * @param {Object} args.parent  - { id (invoice_number), uuid } of parent invoice
 */
export async function GTEInvoiceConsolidatedAdjustmentTemplate({
  consolidatedId,
  type,
  childDocs,
  parent,
