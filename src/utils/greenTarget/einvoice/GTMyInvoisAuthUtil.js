// src/utils/greenTarget/einvoice/GTMyInvoisAuthUtil.js
...
/**
 * Ensures a valid MyInvois API token is available for Green Target
 * @param {Object} apiClientOrConfig - Either the GTEInvoiceApiClient instance or configuration object
...
export async function ensureValidToken(apiClientOrConfig) {
  ...
  const apiClient =
    typeof apiClientOrConfig.makeApiCall === "function"
      ? apiClientOrConfig
      : GTEInvoiceApiClientFactory.getInstance(apiClientOrConfig);
  ...
  try {
    ...
    console.error("Failed to refresh Green Target MyInvois token:", error);
    throw new Error(
