...
> **Update 21 Jul 2026** (legacy-report scan parity, development only — production pending): the **1A-7 opening-balance engine wiring is done** — 642 guarded January anchors (DR = CR RM13,180,681.18) are loaded, the TB/BS engines consume anchors, and the IS/Current Year Profit/CoGM add exact fiscal-year opening stock (3-1/3-3/3-7; CoGM 3-3/3-7). The Jan–May TB residue (RM1,456,480.37) is closed; the May Balance Sheet balances at RM8,980,756.68 with month-end closing stock keyed in `closing_stock_values` and injected at report level (V3), and the Trade Debtor list/statement matches the legacy scans 150/150 including FIFO aging. **Open Questions 1–4 (bottom of this file) are settled by the scans.** Still open and unchanged: Cash Flow, Changes in Equity, comparatives, Schedule B, fixed-asset register/depreciation, HP schedule, tax, period lock, and the 1A-7 bulk setup *screen* (anchors were loaded by guarded migration, not UI). Details: [LEGACY_REPORT_VERIFICATION_PLAN.md](LEGACY_REPORT_VERIFICATION_PLAN.md).
...
| Foreign General Purchase form + manual self-billed e-Invoice submission to MyInvois (FX rate, supporting documents, foreign supplier profiles) *(21 Jul 2026 decision, implemented on dev: foreign self-billed invoices no longer auto-post a `GP` journal — their posting account `OP`/`LGP` carries **no `fs_note`** and the real accounting is the user's separate manual purchase journals; migration `2026-07-21_foreign_gp_unlink.sql` unmapped `OP`/`LGP` and cancelled the existing 56 unpaid foreign journals; harness re-pin + production pending, see [LEGACY_REPORT_VERIFICATION_PLAN.md](LEGACY_REPORT_VERIFICATION_PLAN.md) §8-7)* | ✅ | [GeneralPurchaseInvoiceFormPage](src/pages/Accounting/Purchases/GeneralPurchaseInvoiceFormPage.tsx), [self-billed-invoices.js](src/routes/accounting/self-billed-invoices.js) |
...
| **Payroll Bank Payment (settlement):** salary/director/EPF/SOCSO/SIP/PCB/half-month → `B` journals (DR accrual · CR bank), keyed manually (auto-generation page removed 22 Jul 2026) | ✅ | [JournalEntryPage](src/pages/Accounting/JournalEntryPage.tsx) |
...
All four statements render a single period only — no prior-year comparative column *(= 1A-3, Type 2 / #7)*. Note 5 is shown as one rolled-up number — no Schedule B itemisation *(= 1A-5, Type 2 / #4)*. They originally summed posted journals **YTD from Jan 1 with no brought-forward** and ignored the opening-balance anchor; since 20 Jul 2026 (development, report V2) the TB/BS engines read the latest applicable anchor and the IS/Current Year Profit/CoGM add exact fiscal-year opening stock, so the Balance Sheet balances once the guarded January anchors are loaded *(1A-7 engine wiring done; the bulk setup screen and production rollout remain — see the 21 Jul 2026 update above)*.
...
3. ✅ **Payroll Bank Payment (settlement)** — the dedicated auto-generation screen and its `payroll-payments.js` route were **removed 22 Jul 2026**. Payroll bank-payment settlements (net salary, EPF/SOCSO/SIP/PCB, half-month) are now keyed manually as `B` journals (DR accrual / CR bank) on the Journal Entry page. *Each row settles an accrual, so post the month's `JVSL`/`JVDR` expense vouchers first or the `ACW_*`/`ACD_*` accrual goes negative (harmless for the bank tie-out, messy on the TB).*
...
1. **Opening balances — RESOLVED 20 Jul 2026 (development, report V2).** The TB/BS engines now consume `account_opening_balances` (latest applicable anchor + posted movement) and the IS/Current Year Profit/CoGM add exact `YYYY-01-01` opening stock once; 642 guarded January anchors (DR = CR RM13,180,681.18) are loaded and the Balance Sheet balances. What remains of 1A-7 is the bulk setup *screen* and the production rollout.
2. **Some figures are computed schedules, not natural "entries."** Depreciation (needs the fixed-asset register), HP interest/principal split (needs the amortization schedule), and tax provision / deferred tax. (**Closing-inventory valuation is done** — 21 Jul 2026, V3: exact-month values keyed in `closing_stock_values` on the Material Stock page, injected at report level into BS/IS/CoGM, never a GL posting.) Each *can* be computed in Excel and keyed as a journal — but that is exactly the fragile manual work the ERP exists to remove, and where the numbers drift.
...
| Target | Distance |
|---|---|
| **Monthly pack that ties out** (TB/BS/IS/COGM, 12/2024-style) | **Reached on development 21 Jul 2026** — the May 2026 pack ties to the legacy scans (BS balances RM8,980,756.68). Gating work was: (a) load full opening balances and wire the statements — done 20 Jul 2026 (V2); (b) recurring non-auto journals — General Purchase → GL built 2 Jul 2026, closing stock keyed monthly since 21 Jul 2026 (V3); remaining: depreciation, HP, tax. |
...
**Verdict:** manually keying every journal *would* generate a correct-looking monthly TB→BS→IS→COGM, and with opening balances loaded (done 20 Jul 2026) the May pack now balances on development. You would still be hand-computing depreciation, HP interest and tax outside the system (closing stock is now keyed monthly in the ERP). Pure-manual is a viable *transition tactic*, not a sustainable or audit-complete process. **The next unlocks are the recurring computed journals (depreciation, HP, tax) and the audited-set statements (Cash Flow / Changes in Equity / comparatives).**
...
| 7 | **Opening balance setup** | 🟡 | Per-account anchor `account_opening_balances` + a "Set opening balance" modal now exist (built 9 Jun 2026, first used by the Bank Statement report). (b) **done 20 Jul 2026 (development, report V2)**: the TB/BS/IS/CoGM engines consume the anchor and add exact fiscal-year opening stock; 642 guarded January anchors are loaded and the Balance Sheet balances. Still needed: (a) a full setup *screen* to load every account's DR/CR migration balance at once (the legacy "Opening Balance as at 1 Sept 2012") — anchors so far were loaded by guarded migration SQL, not UI — plus the production rollout of the V2 population. |
...
**Open Questions 1–4** (`THJ_CK`/`THJ_SM`, `NT_7484`, `CL_GT`/`CL_GF` debit balances, `BTRA`) — **settled by the Jan–May 2026 Trial Balance scans (21 Jul 2026)**: `THJ_CK`/`THJ_SM` print APPX 5-1 (factory salaries; no inter-company recharge), `NT_7484` prints APPX 5 with a zero balance (no Note 8 reclass), `CL_GT` DR 12,415.60 / `CL_GF` DR 31,696.82 print APPX 8 (legacy convention kept; V2 applied the fs_note move), and `BTRA` prints APPX 5 (administrative transportation, not CoGM freight-in). Evidence: [LEGACY_TRIAL_BALANCE_CODE_ANALYSIS.md](LEGACY_TRIAL_BALANCE_CODE_ANALYSIS.md) addenda; regression-pinned by the legacy-report harness.
