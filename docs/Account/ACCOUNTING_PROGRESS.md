...
| [GT_ACCOUNTING_HANDOVER.md](GT_ACCOUNTING_HANDOVER.md) | **Green Target accounting build-out + Jan–Jun 2026 legacy import (Phases G0–G8).** Phases G0–G5 complete 27 Jul 2026 — see its §9. The `greentarget` schema holds its own 503-account chart, 34-note catalogue, 1,705 imported legacy journals (4,401 lines) and 501 opening anchors, and the read-only report engines reproduce all six printed Trial Balances, the Income Statement and the Balance Sheet exactly. GT data is fully isolated — no GT row lives in a `public` accounting table. Read its §3a date rule before touching any GT source file. **G6 (frontend) is the critical path.** |
