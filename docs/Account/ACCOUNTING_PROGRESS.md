...
| [GT_ACCOUNTING_HANDOVER.md](GT_ACCOUNTING_HANDOVER.md) | **Green Target accounting build-out + Jan–Jun 2026 legacy import (Phases G0–G8 and current parity phases).** The isolated G4 baseline remains 1,705 imported legacy journals (4,401 lines), 34 notes and the provenance-checked legacy chart/opening set; the report engines reproduce all six printed Trial Balances, the Income Statement and the Balance Sheet. The 30 Jul continuation adds explicit invoice debtor + `TGA`/`TGB`/`WS_OTH` selection, grouped receipt-owned automatic `REC` journals through `PBB_1`, and the evidenced `CD_SD` schedule with 746 real child accounts, July cutover anchors, search/PDF and end-to-end hide-zero filtering. Read §3a before touching a GT source file and use this handover as the detailed phase/gap record. GT remains fully isolated from `public` accounting tables. |
