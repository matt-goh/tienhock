...
- **Follow-ups delivered 6 Aug 2026:** the monthly tie-out tool `dev/import/legacy-tieout/tie-out.mjs` (one command → account-level legacy-vs-ERP diff; June re-run shows exactly one row, the documented CR_LD +40.00 anomaly, and the folded ERP grand total ties to the print at 2 × 17,102,880.87) and the prevention strategy [LEGACY_TIEOUT_STRATEGY.md](LEGACY_TIEOUT_STRATEGY.md) (monthly detection → optional vendor→account memory → single-keying end state).
