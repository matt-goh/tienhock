...
**Posting lock implemented:** Tien Hock sales, ordinary receipt/payment, and adjustment accounting mutations dated before **2026-06-01** fail with HTTP 409 / `ACCOUNTING_PERIOD_LOCKED`. JP is explicitly excluded. Direct SQL, manual journals, and bank-in mutations remain outside this narrow application guard (§9). Since 24 Jul 2026, a single invoice whose payment is already proven by one exact hash-pinned imported REC can be aligned operationally through the explicit imported-ledger confirmation flow; it creates only a non-posting `contra` payment and never changes or duplicates the locked journal.
...
- The posting lock is intentionally application-level and narrow, not a full period close. Direct SQL/migrations, manual journals, bank-ins, purchase invoices, self-billed/general purchases, supplier payments, payroll bank payments, and journal-voucher generation bypass it. A pre-cutoff pending receipt also remains locked even if its proposed clearance date is after the cutoff. The one app-level reconciliation exception is non-posting: an exact imported receipt can clear its operational invoice only after explicit confirmation and strict journal/staging/GL guards; it cannot create, edit, or cancel a locked-period posting.
