...
1. ~~(v1)~~ **Superseded by the v2 pivot below:** JP now has its OWN catalogue (jellypolly.staffs/jobs/pay_codes/mappings/rate schedules/leave). ~~JP membership is managed by `jellypolly.payroll_employees`.~~ **Superseded (2026-07-05):** membership is derived from `jellypolly.staffs.job` (TH-style); `jellypolly.payroll_employees` and the Staff Assignment page were removed (migration `012_jp_staffs_job_membership.sql`).
...
- Membership pivot to staffs.job (2026-07-05): JP page/payroll membership now derives from `jellypolly.staffs.job` (TH-style) instead of `jellypolly.payroll_employees`. Removed: `JPStaffAssignmentPage`, `useJPPayrollEmployees`, the `/jellypolly/api/payroll-employees` route, and the table (standalone migration `dev/migrations/012_jp_staffs_job_membership.sql` folds active assignments into `staffs.job` then drops it — written for prod, which already ran `JP_PROD_DEPLOY.sql`; the deploy script no longer creates the table). Backend consumers (`jpPayrollProcessor.js`, `monthly-work-logs.js`, `daily-work-logs.js` leave entry, `production-entries.js` workers) and all JP pages (monthly/daily/salesman/plastic entry, Payroll, Bonus, Others ×2, Pinjam, Mid-month) now resolve membership via `JP_JOB_TYPE_TO_JOB_IDS` / `staffHoldsJPJob` in `src/configs/jpPayrollJobConfigs.ts` (PRODUCTION→JP_PACKING). Assign jobs on the JP Job page ("Manage Staff") or the staff form.
- JP Job & Pay Codes page (2026-07-05): added `src/pages/JellyPolly/Catalogue/JPJobPage.tsx` (clone of the TH JobPage on `/jellypolly/api/{jobs,job-pay-codes,staffs}`) plus a "Job" nav entry in the JP Catalogue group — the seeded `jellypolly.job_pay_codes` mappings are now manageable in the UI. Shared components gained props for it: `NewJobModal.sectionOptions` (static JP section list; JP has no sections table), `AssociateEmployeesWithJobModal.apiBase`, and `JobsAndEmployeesUsingPayCodeTooltip.navigateBase` (JPPayCodePage now passes `/jellypolly` so tooltip links open JP pages). Also fixed a cross-company server cache collision: `src/routes/jellypolly/{jobs,pay-codes,staffs}.js` now cache under `jp:`-prefixed keys instead of sharing the TH `jobs`/`pay_codes`/`staffs` memory-cache keys.
