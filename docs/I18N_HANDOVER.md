...
**Status:** Phase 0 **DONE 2026-08-04**. B1 (`common`) **DONE 2026-08-04**. B3 (`invoice`) **DONE 2026-08-07** — all 23 files converted (16 in part 1, 7 in part 2); see §9. BM tone audit applied to all existing ms locales. B2 (`auth`, `nav`) **DONE 2026-08-07**. B4 (`payments`/`adjustments`/`sales`) **DONE 2026-08-07**. B5 (`sales`) **DONE 2026-08-07**. B6 (`catalogue`) **DONE 2026-08-07** – all 15 pages converted. B7 (`catalogue`) **DONE 2026-08-07** – all 27 components under `src/components/Catalogue/**` converted; `catalogue` namespace now has 819 keys per language. B8 (`payroll`) **DONE 2026-08-07** – all 24 payroll pages converted; `payroll` namespace now has 694 keys per language. B9 (`payroll`) **DONE 2026-08-07** – all 34 components under `src/components/Payroll/**` converted (9 in part 1, 25 in part 2); `payroll` namespace now has 967 keys per language. B10 (`stock`) **DONE 2026-08-07** – all 12 Stock pages + 11 Stock components converted; `stock` namespace now has 576 keys per language. B11 (`accounting`) **DONE 2026-08-07** – all 22 files under `src/pages/Accounting/**` converted; `accounting` namespace now has 960 keys per language. B12 (`accounting`) **DONE 2026-08-18** – all 10 files under `src/components/Accounting/**` covered (9 converted this batch; `OpeningBalanceModal` was already converted; bilingual BM/EN help dialogs kept raw with app-language default); `accounting` namespace now has 1033 keys per language. B13 (`greentarget`) **DONE 2026-08-18** – first alphabetical half of `src/pages/GreenTarget/**` (23 files: 13 pages converted, 10 Accounting re-export stubs with no own literals); new `greentarget` namespace now has 706 keys per language. **B14** (`greentarget`) **DONE 2026-08-19** – second alphabetical half of `src/pages/GreenTarget/**` (22 pages incl. `InvoiceListPage`, `GreenTargetPaymentPage`, all 15 Payroll pages, `PublicForm/CustomerSignupPage`, `Rentals/*`, `Sales/GTSalesSummaryPage`) + all 14 files under `src/components/GreenTarget/**` converted (incl. the PUBLIC customer signup form, now on i18next); `greentarget` namespace now has 1761 keys per language. **B15/B16** (`jellypolly`) **DONE 2026-08-20** – every JellyPolly page and component converted (33 pages + 2 components + 3 edit wrappers; re-export stubs skipped); `jellypolly` namespace now has 1142 keys per language. B17 (`misc`) **DONE 2026-08-21** – all 12 hooks/utils that emit their own toasts converted; new `misc` namespace has 40 keys per language (see §9). B18 (residue sweep) **DONE 2026-08-21** – App.tsx titles, EditIncentiveModal + PayslipButtons (never actually converted despite B9's log), routes/catalogue/customerValidation, and 6 raw-string/placeholder leftovers fixed (see §9). **Rollout feature-complete — pending the user's manual language-switch smoke test.**

---

## 0. CONTINUE HERE (fresh session)

**Pick up exactly here:** the rollout is feature-complete. Remaining work is the **user's manual smoke test**: run the dev server, switch language in the user menu, confirm navbar + sidebar + HomePage re-render in all 3 languages, spot-check a page per module (esp. payroll payslip print/download buttons, Edit Incentive/Bonus/Advance modal, customer signup form, account-code favourites/bookmarks, invoice creation/editing toasts), confirm a `zh-CN` browser profile defaults to 简体中文 and an unmatched locale (e.g. `de`) defaults to Bahasa Melayu. If anything is found, fix it and append a row to §9.

B17 and B18 are complete (2026-08-21, see §9). B17: `misc` is a live namespace (43 keys per language after B18, registered in `src/i18n/index.ts`); it covered every file under `src/hooks/*.ts`, `src/utils/**` (including the 4 `.tsx` cache hooks) and non-PDF `src/services/*.ts` that emits its own toast/alert. B18 swept `src/App.tsx` (document titles now translated via the `nav` namespace), `index.html` (static `<title>Tien Hock ERP</title>` + meta description kept as proper-noun brand, not translated), the error-boundary-less layout, and every remaining raw toast/JSX/placeholder found by grep — including two files B9's log wrongly listed as converted (`EditIncentiveModal.tsx`, `PayslipButtons.tsx`) and `src/routes/catalogue/customerValidation.ts`.

Pattern notes for future work: React hooks use `useTranslation("<ns>")` (with `t` added to dependency arrays per the B1 note); non-hook modules (`InvoiceUtils` ×2, `customerValidation` ×2, `PayslipManager`) translate via the `i18n` instance — `i18n.t(key, { ns: "misc" })` — because they cannot call the hook. Server-provided error messages inside toasts stay raw; only static fallbacks are translated, and thrown `Error` messages keep English (they are error objects, not UI text). `dev/i18n-extract.mjs` reports a lot of false positives (CSS classes, `console.error` prefixes, comment fragments, Trans `<strong>` fragments) — a candidate is a real leftover only when it is an unwrapped render/placeholder/aria string or a wrapped key missing from the namespace.

`npx tsc --noEmit` is clean (verified 2026-08-21 after B18).

**Current namespace state:** registered in `src/i18n/index.ts`: `common`, `nav` (166 keys/lang), `home`, `invoice`, `auth`, `payments`, `adjustments`, `sales`, `catalogue` (819 keys/lang), `payroll` (1066 keys/lang), `stock` (576 keys/lang), `accounting` (1038 keys/lang), `greentarget` (1766 keys/lang), `jellypolly` (1196 keys/lang), `misc` (43 keys/lang). `en/` stays sparse (semantic `common.*` only). (Note: the counts for `payroll`/`jellypolly` here are the live JSON key counts — earlier batches grew them past the figures printed in the B9/B15 rows without updating this line.)
...
| 2026-08-20 | B15/B16 (`jellypolly`) | Every file under `src/pages/JellyPolly/**` (except re-export stubs) + all files under `src/components/JellyPolly/**`: 4 root pages, 8 Catalogue pages, 20 Payroll pages, 1 Stock page, 2 components (3 edit wrappers included in the 20 payroll pages) | new `jellypolly` namespace: 1142 keys (ms + zh-Hans each) | `DebtorsReportPage`, 3 `AdjustmentDocs` stubs, `JPCP8DPage` (re-export stubs with no own literals) | Registered `jellypolly` in `src/i18n/index.ts`. Moved partial work in `PaymentPage` (was `payments`) and `InvoiceFormPage` (was `invoice`) fully onto `jellypolly`. Heavy payroll/stock/catalogue/greentarget key reuse. Hardcoded Malay de-Malay-ified (Cuti Tahunan/Sakit/Umum/Rawatan -> standardized leave keys, Digenapkan -> Rounding, Jumlah Digenapkan -> Rounded Total, Jumlah Pinjam -> Total Pinjam, Jumlah Masuk Bank -> Total Banked In, Machine Rosak -> Machine Broken, GAJI -> SALARY, 1/2 BULAN -> 1/2 MONTH, etc. with original Malay as the ms value). `i18n:report` green; `tsc --noEmit` clean |

| 2026-08-21 | B17 (`misc`) | `src/hooks/useAccountCodeFavourites.ts`, `src/hooks/useBookmarks.ts`; `src/utils/accounting/useAccountingCache.ts`, `src/utils/catalogue/useCustomerCache.tsx`, `useJobCategoriesCache.tsx`, `useSalesmanCache.tsx`, `src/utils/invoice/useProductsCache.tsx`, `src/utils/payroll/useContributionRatesCache.ts`, `src/utils/invoice/InvoiceUtils.ts`, `src/utils/JellyPolly/InvoiceUtils.ts`, `src/utils/greenTarget/customerValidation.ts`, `src/utils/payroll/PayslipManager.ts` | new `misc` namespace: 40 keys (ms + zh-Hans each) | `SessionService.ts` (no user-facing literals), all PDF services/utils (`*pdf*`, `*PDFMake*`, `PDF/*`, `einvoice/*`, `PrintOverlay`/`PDFHandler`), pure-logic hooks/utils; `src/routes/catalogue/customerValidation.ts` outside B17 globs — left for B18 | Registered `misc` in `src/i18n/index.ts`. React hooks use `useTranslation("misc")` with `t` added to dep arrays (B1 precedent); non-hook modules use the `i18n` instance (`i18n.t(key, { ns: "misc" })`) since they cannot call the hook. Server-provided error messages in toasts stay raw; only static fallbacks are translated; thrown `Error`s keep English. Manual plurals for payslip count (`{{total}} payslip`/`payslips` — var deliberately not named `count`). `i18n:report` green; `tsc --noEmit` clean |

| 2026-08-21 | B18 (residue sweep) | `src/App.tsx` (document titles now translated via `nav`: `{{page}} | {{company}} ERP`), `src/components/Payroll/EditIncentiveModal.tsx` (listed as converted in B9 but had NO `useTranslation` — full conversion), `src/utils/payroll/PayslipButtons.tsx` (full conversion of all 4 buttons), `src/routes/catalogue/customerValidation.ts` (same toasts as the B17 Green Target copy), `src/pages/JellyPolly/Payroll/JPPayrollDetailsPage.tsx` (`Payroll Items` heading), `src/pages/GreenTarget/Invoices/InvoiceDetailsPage.tsx` (`Invoice not found` fallback + `e.g. RV26/06/62` placeholder), `src/pages/GreenTarget/Payroll/GTECarumanPage.tsx` (3 `e.g.` placeholders), `src/components/GreenTarget/GreenTargetPaymentForm.tsx`, `src/pages/GreenTarget/Rentals/RentalDetailsPage.tsx`, `src/pages/GreenTarget/Invoices/InvoiceFormPage.tsx` (`e.g. RV26/06/62` placeholders); keys only: `SupplierPaymentListPage` filter options (labels were already wrapped in `t()` but 5 accounting keys were missing) | keys added: `nav` +3 (`Login`, `PDF Viewer`, `{{page}} | {{company}} ERP`); `payroll` +12 (`EditIncentiveModal` 10 new: `Bonus`, `Deduct as Advance`, `Employee: {{name}} ({{id}})`, `Please enter a description.`, `Please enter a valid amount.`, `Please select a location for {{displayLabelLower}} entries.`, `Update {{displayLabel}}`, `{{displayLabel}} updated successfully!`, `Failed to update {{displayLabelLower}}.`, `e.g., {{displayLabel}}, Bonus` — `Edit {{label}}` and `{{label}} Date` reused; `PayslipButtons` 2 new: `Download {{count}} PDFs`, `Preparing...`); `greentarget` +5 (`Invoice not found`, `e.g. F1234567890Z`, `e.g. 123456-X`, `e.g. 1234567890`, `e.g. RV26/06/62`); `accounting` +5 (`All sources`, `Material Purchases`, `General Purchases`, `Active only`, `Include cancelled`); `jellypolly` +1 (`Payroll Items`); `misc` +3 (`Address is required for e-Invoice compliance`, `Phone number is required for e-Invoice compliance`, `Customer e-Invoice IDs validated successfully (phone number needed)`) | `index.html` static `<title>Tien Hock ERP</title>` + meta description kept as proper-noun brand (per-page titles now translated); `ReportSourceGuide` bilingual BM/EN help dialog (deliberate exclusion per B12); `Rental Tong (A/B)`/`Waste Management` (persisted DB descriptions); bank names; tax classification labels; console.error prefixes; ChangelogModal corpus | Swept with `dev/i18n-extract.mjs` + targeted greps: the only remaining raw toast in `src/` is `JPConsolidatedInvoiceModal`'s `toast.error(\`${error.message}\`)` (pure server-message passthrough, intentionally raw). `EditIncentiveModal`/`PayslipButtons` proved B9's log was inaccurate — they were never converted; both are now complete. `i18n:report` green; `tsc --noEmit` clean |
