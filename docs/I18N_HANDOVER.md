...
After B11/B12: B13–B16 (GreenTarget + JellyPolly) → B17/B18 (misc + residue). Follow the batch table in §6.

**Current namespace state:** registered in `src/i18n/index.ts`: `common`, `nav`, `home`, `invoice`, `auth`, `payments`, `adjustments`, `sales`, `catalogue` (819 keys/lang), `payroll` (967 keys/lang), `stock` (576 keys/lang). `en/` stays sparse (semantic `common.*` only).
