...
**Status:** Phase 0 **DONE 2026-08-04**. B1 (`common`) **DONE 2026-08-04**. B3 (`invoice`) **DONE 2026-08-07** — all 23 files converted (16 in part 1, 7 in part 2); see §9. BM tone audit applied to all existing ms locales. B2 (`auth`, `nav`) **DONE 2026-08-07**. B4 (`payments`/`adjustments`/`sales`) **DONE 2026-08-07**. B5 (`sales`) **DONE 2026-08-07**. B6 (`catalogue`) **DONE 2026-08-07** – all 15 pages converted. B7 (`catalogue`) **DONE 2026-08-07** – all 27 components under `src/components/Catalogue/**` converted; `catalogue` namespace now has 819 keys per language. B8 (`payroll`) **DONE 2026-08-07** – all 24 payroll pages converted; `payroll` namespace now has 694 keys per language. B9 (`payroll`) **DONE 2026-08-07** – all 34 components under `src/components/Payroll/**` converted (9 in part 1, 25 in part 2); `payroll` namespace now has 967 keys per language. B10 (`stock`) **DONE 2026-08-07** – all 12 Stock pages + 11 Stock components converted; `stock` namespace now has 576 keys per language. B11 (`accounting`) **DONE 2026-08-07** – all 22 files under `src/pages/Accounting/**` converted; `accounting` namespace now has 960 keys per language. B12 (`accounting`) **DONE 2026-08-18** – all 10 files under `src/components/Accounting/**` covered (9 converted this batch; `OpeningBalanceModal` was already converted; bilingual BM/EN help dialogs kept raw with app-language default); `accounting` namespace now has 1033 keys per language. B13 (`greentarget`) **DONE 2026-08-18** – first alphabetical half of `src/pages/GreenTarget/**` (23 files: 13 pages converted, 10 Accounting re-export stubs with no own literals); new `greentarget` namespace now has 706 keys per language. **B14** (`greentarget`) **DONE 2026-08-19** – second alphabetical half of `src/pages/GreenTarget/**` (22 pages incl. `InvoiceListPage`, `GreenTargetPaymentPage`, all 15 Payroll pages, `PublicForm/CustomerSignupPage`, `Rentals/*`, `Sales/GTSalesSummaryPage`) + all 14 files under `src/components/GreenTarget/**` converted (incl. the PUBLIC customer signup form, now on i18next); `greentarget` namespace now has 1761 keys per language. Next: **B15** (JellyPolly).

---

## 0. CONTINUE HERE (fresh session)

**Pick up exactly here:** start **B15** – `src/pages/JellyPolly/**` (all pages + components), namespace `jellypolly`. The JellyPolly pages are mostly thin wrappers over the shared Home/Payroll/Stock/Catalogue/Accounting pages, so most files are either re-export stubs (no own literals) or small bespoke pages — run the extractor per subfolder to see the real surface first.

`greentarget` is a live namespace (706 keys per language, registered in `src/i18n/index.ts`). Reuse existing keys where possible – many component/page strings already exist in `greentarget` or other namespaces (each namespace still needs its own copy). Remember the extractor glob quirk: `**/*.tsx` does NOT match root-level files, so run the extractor once per subfolder (or per file) to cover the whole batch.

Run `node dev/i18n-extract.mjs --ns greentarget --glob "src/pages/GreenTarget/*.tsx"` and `node dev/i18n-extract.mjs --ns greentarget --glob "src/components/GreenTarget/*.tsx"` (and once per nested subfolder) before starting to see which candidate strings are missing.

`npx tsc --noEmit` is currently clean — the pre-existing `AccountLedgerPage.tsx` error from the staged accounting work has since been fixed/committed (verified 2026-08-18).

After B15: B16 (JellyPolly) → B17/B18 (misc + residue). Follow the batch table in §6.

**Current namespace state:** registered in `src/i18n/index.ts`: `common`, `nav`, `home`, `invoice`, `auth`, `payments`, `adjustments`, `sales`, `catalogue` (819 keys/lang), `payroll` (967 keys/lang), `stock` (576 keys/lang), `accounting` (1033 keys/lang), `greentarget` (1761 keys/lang). `en/` stays sparse (semantic `common.*` only).

**B11 notes (2026-08-07):** all 22 files under `src/pages/Accounting/**` converted (+960 keys/lang). Two files already had committed partial i18n work and were moved fully onto the `accounting` namespace: `JournalDetailsPage.tsx` (was partially on `adjustments`; the shared keys — Related journals for Invoice {{id}}, Sales, Credit Note/Debit Note/Refund Note, View journal entry, Posted/Cancelled — were copied into `accounting` so the file uses one namespace) and `Reports/TrialBalancePage.tsx` (was on `common` for "Rows per page"). Bilingual help dialogs with their own BM/EN toggle (`LocationAccountMappingsPage`, `VoucherGeneratorPage`) stay raw and now default to the app language (ms → BM, else EN) like `ProductionHelpDialog`. Hardcoded Malay in `AccountCodeFormPage`/`LocalGeneralPurchaseFormPage` was de-Malay-ified to English keys with the original Malay kept as the `ms` value. Left raw on purpose: e-Invoice payload fields (transaction_type/shipping_method values like "Importation of goods"/"TAOBAO AIR SHIPPING"), statutory acronyms (EPF/SOCSO/SIP/PCB/TIN/SST/MSIC), account descriptions shown in tooltips, bank names, payment-method/status keys kept as English-as-key ("bank transfer"/"cash"/"cheque"/"online", "posted"/"cancelled"/"pending"/"active"), mapping-type chip abbreviations (Comm-MEE, Acc-EPF, CT, SOT...), file sizes, dates, console.error strings. `i18n:report` green; `tsc --noEmit` clean except the pre-existing `AccountLedgerPage.tsx` error. During the batch a map callback parameter named `t` shadowed the i18n `t` in `AccountLedgerPage` and was renamed to `tx` — the only tsc error introduced and fixed. Trans used for sentences with inline markup (Warning banner in `JournalDetailsPage`, direct-posted amounts in `AccountCodeFormPage`, New-item hint in `LocalGeneralPurchaseFormPage`; the latter's `i18nKey` uses a JS expression because JSX attribute strings cannot process `\u2014` escapes). Manual plurals kept as flat keys (never i18next `count`): `{{count}} customer/customers`, `{{count}} invoice/invoices`, `{{count}} account/accounts`, `{{count}} item/items`, `{{count}} change/changes`, `{{count}} unsaved change/changes`, `{{count}} invalid amount/amounts`, `{{count}} voucher/vouchers`, `Save {{count}} change/changes`, `Discard {{count}} unsaved change/changes?`, `Cheque {{chequeNo}} ... 1 other entry/{{count}} other entries`, `Generated {{count}} voucher/vouchers`, `{{count}} voucher already exists/vouchers already exist`, `{{count}} selected invoice will be skipped/invoices will be skipped`, `You are about to submit {{count}} eligible foreign purchase/purchases`.

**B12 notes (2026-08-18):** all 10 files under `src/components/Accounting/**` are now covered (9 converted in this batch; `OpeningBalanceModal` was already fully converted). `accounting` grew 983 → 1033 keys/lang. `TrialBalanceOrderButton`/`TrialBalanceOrderModal` (staged work, previously on `common`) moved fully onto `accounting`; `t("cancel")`/`t("save")` became `t("Cancel")`/`t("Save")`, already in the namespace. Bilingual BM/EN help dialogs (`ReportSourceGuide`, `SelfBilledEligibilityDialog`, `SellerTypeHelpDialog`) stay raw with their own toggles and now default to the app language (ms → BM, else EN) like `ProductionHelpDialog`; `ReportSourceGuide` still persists the manual choice in `report_guide_lang`. `validateSupplierPaymentDraft` gained an optional `t: TFunction` last parameter (backward-compatible; both callers now pass it) so toast errors translate. Hardcoded Malay de-Malay-ified in `AccountCodeCombobox`: `Padanan dipaparkan bersama laluan akaun induk` → key `Matches shown with the parent account path` with the original Malay as the `ms` value. Manual plurals kept as flat keys (never i18next `count`): `{{count}} child/children`. Trans used for the cheque-reuse warning sentence (inline `<strong>` around the cheque number; the count variable is named `n`, not `count`, so plural-key resolution stays off). Left raw on purpose: bank names (`Public Bank`/`Alliance Bank`), `MYR`, payment-method API values, DB content (account descriptions, stock-item names), date formats, console.error strings. `i18n:report` green; `tsc --noEmit` clean — including the pre-existing `AccountLedgerPage.tsx` error, which is gone now that the staged accounting work it belonged to has been committed.

**B13 notes (2026-08-18):** first alphabetical half of `src/pages/GreenTarget/**` (23 files: 13 pages converted, 10 Accounting re-export stubs with no own literals). New `greentarget` namespace registered in `src/i18n/index.ts`; 706 keys/lang (254 exact-key reuses from other namespaces, rest freshly translated; BM terms follow the tone rule — e.g. `Dumpster` = `Tong`, `Rental` = `Sewaan`). Two invoice pages had committed partial i18n work on `nav` (`InvoiceDetailsPage`, `InvoiceFormPage` with a stray `t("Delivery Order")`) and were moved fully onto `greentarget`. Hardcoded Malay de-Malay-ified: `Jumlah pelarasan` → key `Adjustment total`, `Gagal menyediakan identiti penghutang.` → key `Failed to prepare the debtor identity.`. Trans used for sentences with inline markup (picker hint in `GTAdjustmentDocsFormPage`, cancelled banner in `GTAdjustmentDocsDetailsPage`). Manual plurals kept as flat keys (never i18next `count`): `{{count}} rental/rentals`, `Rental Details ({{count}} rental/rentals)`, `Selected: {{count}} rental/rentals`, `Selected Rentals ({{count}})`, `Showing {{start}}-{{end}} of {{count}} rental/rentals`, `Customers ({{count}})`, `Dumpsters ({{count}})`, `({{index}} of {{count}})`. Status/method labels translated at the render sink only (payment methods `cash`/`cheque`/`bank transfer`/`online`, refund methods, dumpster statuses, GT display statuses); values stay English for logic. Left raw on purpose: bank names, `MYR`/`RM`, `MyInvois Portal`, DN/CN/RN tab abbreviations, persisted line descriptions (`Rental Tong (A)`/`Rental Tong (B)`/`Waste Management`, refund excess wording), example placeholders (`e.g. RV26/06/62`, `DO-00123`, `C21636482050`), `N/A`/`Invalid Date`, thrown Error messages, console.error strings. `validateInvoiceNumber` in `InvoiceFormPage` uses conditional keys (`Invoice number already exists` vs `(ID: {{id}})`) instead of string concatenation. `i18n:report` green; `tsc --noEmit` clean.

**B14 notes (2026-08-19):** second alphabetical half of `src/pages/GreenTarget/**` (22 pages) + all 14 files under `src/components/GreenTarget/**` converted; `greentarget` grew 706 → 1761 keys/lang. The PUBLIC customer signup form (`PublicForm/CustomerSignupPage`) was moved off its framework-free `translations.ts` dictionary onto i18next (its own BM/EN/中文 switcher now calls `i18n.changeLanguage`; the old `translations.ts` was deleted). Files with committed partial work on other namespaces were moved fully onto `greentarget`: `InvoiceListPage`/`RentalDetailsPage` (were `nav`), `GTSalesSummaryPage` (was `sales`), `GTPayRatesPage`/`GTEditPayCodeRatesModal` (were `common`), `GTStatementModal` (was `sales`); their needed keys were copied across. Heavy reuse from `payroll` (967 keys) and `common` — payroll-style pages keep the same English-as-key labels (`SALARY`/`GAJI`, `STAFF NAME`/`NAMA PEKERJA`, `1/2 MONTH`/`1/2 BULAN`, `GROSS PAY`, `NET PAY`, `TOTAL`, `ROUNDING`, `AFTER ROUNDING`, `GRAND TOTAL`, `SUBTOTAL`, `MAJ`, `PKJ`) with the original Malay as the `ms` value. Hardcoded Malay de-Malay-ified (original Malay kept as `ms` value): `GTInvoiceAccountFields` (`Identiti Penghutang` → `Debtor Identity`, `Kod Identiti` → `Identity Code`, availability messages), `GTRevenueSplitEditor` (`Agihan hasil` → `Revenue Allocation`, `Tambah baris` → `Add row`, `Telah diagih` → `Allocated`, `Baki` → `Balance`, status messages), `GTDailyLoriHabukEntryPage`, `GTCutiReportPage` legend moved to standardized `Annual Leave`/`Sick Leave`/`Public Holiday`/`Medical Leave` keys, `GTLeaveSection` `GT_LEAVE_TYPES` labels switched from BM to the standardized English leave-type keys. Trans used for sentences with inline markup (RentalDetailsPage "Invoice for…" intro, CutiReport legend). Manual plurals kept as flat keys (never i18next `count`): `{{count}} rental/rentals`, `+{{count}} more rental/rentals`, `{{count}} record/records`, `{{count}} invoice/invoices`, `{{count}} invoice/invoices selected`, `Selected: {{count}} customer/customers`, `{{count}} year/years`, `({{count}} employee/employees)`, `{{count}} invoice/invoices · {{amount}}`, `{{count}} invoice/invoices added to receipt {{reference}}`, `{{count}} selected • Total: {{amount}}`. `getLeaveTypeLabel`/`getStatusLabel`/`formatPaymentMethod`/`formatReceiptDate` stay English module-level helpers and are translated at the render sink via `t(...)`. One shadowing bug was introduced then fixed (map callback local `t` in `RentalFormPage` renamed to `note`). Left raw on purpose: statutory acronyms (EPF/SOCSO/SIP/PCB/SKBBK, MAJ/PKJ), bank names (`Public Bank`), `RM`/`MYR`, persisted line descriptions (`Rental Tong (A)` etc.), example placeholders (`e.g. RV26/06/62`, `DO-00123`, `TH`, `180`), DB content (pay-code/account descriptions, rule condition fields), export/bank file contents, thrown Error messages, console.error strings. `i18n:report` green; `tsc --noEmit` clean.
...
**B10 notes (2026-08-07):** all 12 Stock pages + 11 Stock components converted; `stock` namespace registered with 576 keys/lang. `StockAdjustmentEntryPage` (the ~4,500-line Material/General Stock page) was the heaviest file: module-level label maps (`categoryLabels`, `closingStockFields`, `TAB_CONFIG`, `categoryLabels` in `MaterialsListPage`) stay raw and are translated at the render sink; helpers like `renderRowSaveButton`/`renderAdjustmentValue` receive translated labels from their callers. Non-ASCII edits (middle dot `·`, em dash `—`, multiplication sign `×`) were done with Node `\u`-escape replacements per the gotchas above. De-Malay-ified hardcoded Malay: `Mesin Rosak` → key `Machine Broken`, `Baki Permulaan` → `Opening Balance`, `perkerja` → `workers`, `Karung Hancur - Timbang` → `Karung Hancur (Sack Weighing)`, and the Malay toasts in `ProductStockMovementPage`/`ProductionEntryPage` got English keys with the original Malay as the `ms` value. `ProductionHelpDialog` keeps its own BM/EN toggle (bilingual content, like `ChangelogModal`); its default language now follows the app language (ms → BM, everything else → EN) so zh users land on English. Left raw on purpose: unit labels (`bags`/`kg`/`sack`/`pcs`/`ctn`), stock jargon headers (`ADJ+`/`ADJ-`/`FOC`/`B/F`/`C/F`), pay-code/product IDs (`BH_HANCUR`, `TIMBANG_HANCUR`, `RAMBU`), date displays, console.error strings, and `BundleEntrySection`'s unused `TAB_CONFIG` descriptions. Manual plurals kept as flat keys (never i18next `count`): `{{count}} line/lines`, `{{count}} member/members`, `{{count}} unsaved change/changes`, `{{count}} variant(s)`, `{{count}} worker(s)` etc. `i18n:report` green; `tsc --noEmit` clean except the pre-existing `AccountLedgerPage.tsx` error.
...
| B12 | `accounting` | `src/components/Accounting/**` | 10 |
| B13 | `greentarget` | `src/pages/GreenTarget/**` — half 1 (alphabetical) | 23 |
...
| 2026-08-07 | B10a (`stock`) | All 12 files under `src/pages/Stock/**` (incl. nested `Materials/` and `Reports/`): `GeneralStockPage`, `MaterialStockPage`, `MaterialFormPage`, `MaterialsListPage`, `StockAdjustmentEntryPage`, `ProductionListPage`, `ProductionEntryPage`, `ProductStockAdjustmentEntryPage`, `ProductStockMovementPage`, `EstimatedPLPage`, `EstimatedUnitCostPage`, `EstimatedReportPage` | new `stock` namespace created + registered in `src/i18n/index.ts`; 576 keys per language after the full batch (shared with B10b) | `MaterialStockPage`, `EstimatedPLPage`, `EstimatedUnitCostPage` (no own literals) | `StockAdjustmentEntryPage` is the largest file in the rollout (~4,500 lines) and is fully converted. Module-level label maps translated at the render sink; hardcoded Malay de-Malay-ified (Mesin Rosak -> Machine Broken, Baki Permulaan -> Opening Balance, etc.). `npm run i18n:report` green; `tsc --noEmit` clean except pre-existing `AccountLedgerPage.tsx` error |
| 2026-08-07 | B10b (`stock`) | All 11 files under `src/components/Stock/**`: `BundleEntrySection`, `EstimatedReportMappingModal`, `GeneralStockCategoryModal`, `HancurEntrySection`, `MaterialAccountMappingModal`, `ProductionHelpDialog`, `ProductPayCodeMappingModal`, `ProductSelector`, `ProductStockMovementTable`, `WorkerEntryGrid`, `WorkerPayCodeEntryGrid` | keys added to shared `stock` namespace (576 keys per language after batch) | `ProductionHelpDialog` (keeps its own BM/EN toggle; default now follows the app language so zh lands on English) | Sub-components carry their own hooks; module-level helpers (`describeDraftSource`, `formatCategory`) take `t` as a parameter; `ProductSelector` placeholder default resolved at the render sink. `i18n:report` green; `tsc --noEmit` clean except pre-existing `AccountLedgerPage.tsx` error |
| 2026-08-07 | B11 (`accounting`) | All 22 files under `src/pages/Accounting/**` (10 root + 7 `Purchases/` + 5 `Reports/`): `AccountCodeFormPage`, `AccountCodeListPage`, `BankInPage`, `DebtorsReportPage`, `JournalDetailsPage`, `JournalEntryListPage`, `JournalEntryPage`, `LocationAccountMappingsPage`, `OpeningBalancesPage`, `VoucherGeneratorPage`, `Purchases/GeneralPurchaseInvoiceFormPage`, `Purchases/GeneralPurchaseInvoiceListPage`, `Purchases/LocalGeneralPurchaseFormPage`, `Purchases/SupplierFormPage`, `Purchases/SupplierPaymentFormPage`, `Purchases/SupplierPaymentListPage`, `Purchases/SuppliersListPage`, `Reports/AccountLedgerPage`, `Reports/BalanceSheetPage`, `Reports/CogmPage`, `Reports/IncomeStatementPage`, `Reports/TrialBalancePage` | new `accounting` namespace: 960 keys (ms + zh-Hans each) | Bilingual help dialogs in `LocationAccountMappingsPage`/`VoucherGeneratorPage` (own BM/EN toggle; default now follows the app language) | Registered `accounting` in `src/i18n/index.ts`. Moved the committed partial work in `JournalDetailsPage` (was `adjustments`) and `TrialBalancePage` (was `common`) fully onto `accounting`. See B11 notes above for de-Malay-ified strings, Trans usage, manual plurals and raw-on-purpose items. `npm run i18n:report` green; `tsc --noEmit` clean except the pre-existing `AccountLedgerPage.tsx` error |
| 2026-08-18 | B12 (`accounting`) | 9 of 10 files under `src/components/Accounting/**` converted (`AccountCodeCombobox`, `ChequeReuseWarning`, `GeneralStockItemCombobox`, `SupplierPaymentInlineSection`, `TrialBalanceOrderButton`, `TrialBalanceOrderModal`, `ReportSourceGuide`, `SelfBilledEligibilityDialog`, `SellerTypeHelpDialog`); `OpeningBalanceModal` already converted; 2 caller pages updated | accounting +50 keys (983 → 1033 per language) | Bilingual BM/EN help dialogs (`ReportSourceGuide`, `SelfBilledEligibilityDialog`, `SellerTypeHelpDialog`) kept raw; bank names (`Public Bank`/`Alliance Bank`) left raw per §4 | `TrialBalanceOrderButton`/`TrialBalanceOrderModal` moved from `common` onto `accounting`; `validateSupplierPaymentDraft` gained an optional `t` param; hardcoded Malay in `AccountCodeCombobox` de-Malay-ified. `i18n:report` green; `tsc --noEmit` clean (pre-existing `AccountLedgerPage.tsx` error now gone) |
| 2026-08-18 | B13 (`greentarget`) | 13 pages under `src/pages/GreenTarget/**` half 1 converted (`GTDebtorSubSchedulePage`, `GTVoucherGeneratorPage`, `GTAdjustmentDocsDetailsPage`, `GTAdjustmentDocsFormPage`, `GTAdjustmentDocsListPage`, `CustomerFormPage`, `CustomerListPage`, `DebtorsReportPage`, `DumpsterFormPage`, `DumpsterListPage`, `GreenTargetDashboardPage`, `Invoices/InvoiceDetailsPage`, `Invoices/InvoiceFormPage`); 10 Accounting re-export stubs skipped (no own literals) | new `greentarget` namespace: 706 keys (ms + zh-Hans each) | 10 Accounting stub files (no own literals) | Registered `greentarget` in `src/i18n/index.ts`; 254 keys reused from other namespaces. `InvoiceDetailsPage`/`InvoiceFormPage` moved from partial `nav` work fully onto `greentarget`. Hardcoded Malay de-Malay-ified (`Jumlah pelarasan` -> `Adjustment total`, `Gagal menyediakan identiti penghutang.` -> `Failed to prepare the debtor identity.`). `i18n:report` green; `tsc --noEmit` clean |
| 2026-08-19 | B14 (`greentarget`) | 22 pages under `src/pages/GreenTarget/**` half 2 converted (`Invoices/InvoiceListPage`, `Payments/GreenTargetPaymentPage`, `Payroll/` all 15 files, `PublicForm/CustomerSignupPage`, `Rentals/RentalDetailsPage`, `Rentals/RentalFormPage`, `Rentals/RentalListPage`, `Sales/GTSalesSummaryPage`) + all 14 files under `src/components/GreenTarget/**` | `greentarget` +1055 keys (706 -> 1761 per language) | 10 Accounting re-export stubs (no own literals; already skipped in B13) | Public signup form moved off its framework-free `translations.ts` onto i18next (`translations.ts` deleted). Partial-work files moved fully onto `greentarget` (`InvoiceListPage`/`RentalDetailsPage` from `nav`, `GTSalesSummaryPage`/`GTStatementModal` from `sales`, `GTPayRatesPage`/`GTEditPayCodeRatesModal` from `common`). Heavy payroll/common key reuse; hardcoded Malay de-Malay-ified in `GTInvoiceAccountFields`, `GTRevenueSplitEditor`, `GTDailyLoriHabukEntryPage`, `GTLeaveSection` (leave-type labels standardized). `i18n:report` green; `tsc --noEmit` clean |
