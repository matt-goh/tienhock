...
- `account_opening_balances` - id, account_code (FK account_codes), as_of_date, amount (signed; DR-positive for assets), notes, created_at, updated_at, created_by (unique: account_code, as_of_date). GL opening-balance anchor: when a row exists with as_of_date <= a report period start, the Bank Statement report seeds opening = amount + sum of posted lines in [as_of_date, period_start) and ignores everything before as_of_date. Generalises to the migration Opening Balance setup (gap 1A-7).
...
- `payments` - payment_id, invoice_id, payment_date, amount_paid, payment_method (cash/cheque/bank_transfer/online, plus migration-controlled `contra` for applying credit already present in the debtor ledger; contra rows are non-posting and cannot be cancelled as cash payments), payment_reference, internal_reference, bank_account (CASH/BANK_PBB/BANK_ABB), journal_entry_id (FK journal_entries; legacy pre-cutover REC journal only — post-cutover rows are NON-POSTING projections), notes, created_at, status, cancellation_date, cancellation_reason, is_auto_collection (true = automatic CASH-bill collection row owned by the invoice; non-posting, adjusted/cancelled only by invoice edits/conversions — never infer from notes), receipt_allocation_id (FK receipt_allocations; set = row is the display projection of a receipt allocation, managed by the receipt lifecycle; single-allocation receipts cancel transparently through the payment cancel endpoint, grouped ones must cancel the receipt). Since Phase 2 the TH receipt journals belong to `receipts`; `payments` remains the per-invoice payment-history view
...
- `employee_payrolls` - id, monthly_payroll_id, employee_id, job_type, section, gross_pay, net_pay, created_at, updated_at (set on each process/edit; powers the Payroll page "Recent" view recency fallback), employee_job_mapping, digenapkan, setelah_digenapkan, ot_calculation (nullable JSONB; July-2026+ OT salary-formula audit snapshot — version, pay basis, numerator breakdown, excluded bonus/OT/leave (paid leave is NOT in the OT numerator; HR prices leave from the derived daily rate), divisor days, daily/hourly rates, 1.5/2.0/3.0 rates — written by the processors via `buildOTSnapshot` in `src/routes/payroll/otFormula.js`; same column on greentarget.employee_payrolls and jellypolly.employee_payrolls)
...
- `monthly_work_log_entries` - id, monthly_log_id, employee_id, job_id, total_hours (regular/Biasa hours), overtime_hours (regular/Biasa OT), ahad_hours (Ahad non-OT), ahad_overtime_hours (Ahad OT), umum_hours (Umum non-OT), umum_overtime_hours (Umum OT), worked_days (nullable numeric(4,1); manual actual-worked-days input for July-2026+ OT formula actual_days staff whose dates aren't reconstructible from monthly hours; same column on greentarget.monthly_work_log_entries and jellypolly.monthly_work_log_entries), created_at
