...
- `greentarget.customer_signups` - signup_id (PK), name (required), id_number (required for new submissions), phone_number (required for new submissions), address (legacy first-address compatibility), locations (JSONB snapshot array of required site+address pairs; 1-20 for new submissions), payment_method (cash/online/qr), einvoice_requested, tin_number, id_type, email, state, einvoice_validated_at, status (pending/processed/rejected; default pending), customer_id (nullable FK greentarget.customers ON DELETE SET NULL; set on convert), submitted_at, processed_at, processed_by, submitted_ip. Review queue for the PUBLIC (unauthenticated) Green Target registration form served on `greentarget.tienhock.com` and `/greentarget-form`; list/convert/status staff endpoints require a valid staff session. The e-Invoice group is hidden by default; when requested, ID Type, ID Number and TIN are required and validated with Green Target's MyInvois credentials before intake, email is optional, and state is fixed to Sabah (`12`) by the server. Staff conversion atomically inserts the customer and every snapshotted location, then marks the request processed (`src/routes/greentarget/customer-signups.js`)
