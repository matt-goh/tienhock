...
- `greentarget.customer_signups` - signup_id (PK), name (required), id_number, phone_number, address, payment_method (cash/online/qr), status (pending/processed/rejected; default pending), customer_id (nullable FK greentarget.customers ON DELETE SET NULL; set on convert), submitted_at, processed_at, processed_by, submitted_ip. Review queue for the PUBLIC (unauthenticated) Green Target customer registration form served on `greentarget.tienhock.com` and at `/greentarget-form` (see `src/pages/GreenTarget/PublicForm/CustomerSignupPage.tsx`). Public submit endpoint `POST /greentarget/api/customer-signups` (in-memory rate limited, 5/10min/IP); staff review + one-click `POST /:id/convert` (transactional: inserts greentarget.customers + greentarget.locations for the address, marks signup processed) on the GT Customers → Signup Requests page (`src/routes/greentarget/customer-signups.js`)
