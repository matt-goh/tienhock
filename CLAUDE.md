...
- `leave_records` - id, employee_id, leave_date, leave_type ('cuti_umum' | 'cuti_sakit' | 'cuti_tahunan' | 'cuti_rawatan'), work_log_id (TH daily log FK), jp_work_log_id (JP daily log FK), company (TH/JP; payroll payment scope only), days_taken, amount_paid, status, notes, created_by, created_at, updated_at. Leave entitlement/balance usage remains shared by staff name across companies; TH payroll pays `company='TH'` rows and JP payroll pays `company='JP'` rows so dual-company staff are not double-paid for leave.
...
**Jelly Polly Payroll (jellypolly schema; migrations `dev/migrations/008_jp_payroll_foundation.sql` + `009_jp_seed_mock_data.sql` + `010_jp_leave_link.sql` + `011_jp_leave_company.sql`; production bundle `dev/migrations/JP_PROD_DEPLOY.sql`):**

JP shares the public catalogues (staffs, pay_codes, jobs, job_pay_codes, employee_pay_codes, pay_rate_schedules); JP jobs/pay codes are `JP_`-prefixed rows there (section "JELLY POLLY"). Mock/test staff seeded with ids `JPT_*` and names ending "(JP TEST)" — delete with `DELETE FROM public.staffs WHERE id LIKE 'JPT_%'` when real data is entered. JP payroll data lives in the jellypolly schema:

- `jellypolly.payroll_employees` - id, employee_id (FK public.staffs), job_type (OFFICE/MAINTENANCE/SALESMAN/SALESMAN_IKUT/ICE_POLLY/JELLY_CUP/PLASTIC/PRODUCTION), date_added, is_active, notes (unique: employee_id, job_type). User-managed staff→page assignments; unlike GT, one employee may hold multiple job types. Backs all JP payroll/daily/production page staff lists (no hardcoded lists).
- `jellypolly.monthly_payrolls` - id, year, month, created_at, updated_at, created_by (unique: year, month)
- `jellypolly.employee_payrolls` - id, monthly_payroll_id (FK CASCADE), employee_id (FK public.staffs), job_type, section, gross_pay, net_pay, created_at, updated_at, employee_job_mapping, digenapkan, setelah_digenapkan (unique: monthly_payroll_id, employee_id — JP reprocesses per employee on every save; rows exist only for canonical HEAD ids, sub-ID work carries source_employee_id on items)
- `jellypolly.payroll_items` - id, employee_payroll_id (FK CASCADE), pay_code_id (FK public.pay_codes), description, rate, rate_unit, quantity, foc_units, amount, is_manual, created_at, job_type, source_employee_id, source_date, work_log_id, work_log_type (TH shape; no GT rental columns)
- `jellypolly.payroll_deductions` - id, employee_payroll_id (FK CASCADE), deduction_type (epf/socso/sip/income_tax), employee_amount, employer_amount, wage_amount, rate_info, created_at
- `jellypolly.monthly_work_logs` / `_entries` / `_activities` - TH shape incl. ahad/umum hour columns and activity description (Office/Maintenance monthly entry; unique log: log_month, log_year, section)
- `jellypolly.daily_work_logs` / `_entries` / `_activities` - TH shape (Salesman, Ice-Polly, Jelly Cup, Plastic daily entries). Machine-page `bungkusan` is stored in the log header's context_data (no pay code). JP daily leave records are written to shared `public.leave_records` with `jp_work_log_id` and `company='JP'`; deleting a JP daily log cascades its linked leave rows.
- `jellypolly.pinjam_records`, `jellypolly.mid_month_payrolls`, `jellypolly.commission_records` (Bonus/Advance via is_advance), `jellypolly.others_records` (Kerja Luar OT) - GT-shaped clones scoped to JP so JP records stay out of TH/GT lists
