...
- `greentarget.locations` - location_id, customer_id (FK greentarget.customers ON DELETE CASCADE), site (short location label, usually one word; all distinct Sites from an invoice's rentals are appended after its primary billing address in individual Green Target sales and adjustment e-Invoices), address, phone_number. Existing pre-site rows may have a NULL site until staff backfills them
